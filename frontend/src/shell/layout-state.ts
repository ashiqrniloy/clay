...
// the workspace view's right rail and the agent view's inspector. Both are
// per-tab layout state (DESIGN.md §5 "layout state persists per surface";
// plan 118 task 33's acceptance criteria), so the values are keyed by the tab's
// clientId and mirrored into `layout.json` by the workspace controller — the
// store itself stays dependency-free so the titlebar, the chords, the rail and
// the agent panel all read one owner instead of threading props.
