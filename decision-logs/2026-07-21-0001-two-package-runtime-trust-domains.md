...
## Appended note (2026-09-20, plan 131 dead-code sweep)

Rule 4's typed cross-domain envelope contract was implemented ahead of its consumer and never acquired one, so plan 131 deleted it: `src/server/cross_domain.rs` (module file, `mod` declaration, `CrossDomainRequestEnvelope`/`CrossDomainResultEnvelope`/closed `CrossDomainStatus`, `validate_cross_domain_request`, validator tests) plus its `CROSS_DOMAIN_PAYLOAD_BUDGET_BYTES` budget constant. It was pre-wired for plan 061 task 8's first-party extension-point handler surfaces, which have not arrived; per the review's §7 dead-code table and §10 A4, "wire it up later" is not a plan. The decision itself is unchanged and remains binding — no V8 objects, functions, promises, globals, or module instances cross domains, cross-domain work stays Rust-mediated with bounded inert JSON, and the live paths are the third-party load bridge (`op_clay_packages_load_in_package_domain` → `dispatch_to_domain`) with `absorb_cross_domain_evaluation` merging coordinator-bound registrations. Any future cross-domain invocation re-adds the validator together with the handler that consumes a validated route (git retains the implementation at this commit), rather than shipping another unwired envelope. Durable design record: `docs/reference/primitives/package-security.md` and the `CrossDomainRequestEnvelope` row in `docs/reference/primitives/registry.md` (now `Deferred`).
