...
- `src/server/workspace/mod.rs:1871-1940` (`check_openable_size`, `read_file_bounded`): the gate and bounded read to replace; `src/server/workspace/mod.rs:2209-2245` (`save_io`): whole-document `String` materialization to replace with streamed rope chunks.
- `src/server/workspace/mod.rs:1254-1267` (`open_document_snapshots`): runtime-generation reload materializes every open document's full text simultaneously. This is an internal refresh path, not frontend bootstrap; it is converted to metadata plus bounded rope-derived classification/parse/analysis inputs.
