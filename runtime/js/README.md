...
`src/server/facades.rs` includes these checked-in JavaScript files at compile time and classifies every module as trusted-only or public to the shared third-party runtime. The table currently admits 23 facade modules, 14 public to third-party packages. `ClayModuleLoader` resolves only rows admitted by that table. There is no embedded raw-string copy in `src/server/js_runtime.rs` and no runtime filesystem read or transpilation.
