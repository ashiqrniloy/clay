...
6. Stateful kinds (Steps 12–13) — scroll via `Portal` (12), then the transient family (dropdown/collapse/modal/textInput) (13); both rely on Step 11 to persist state across updates. Each deletes its legacy hit-test/interaction code.
...
| 13 | Transient family → real widgets (`overlay`/`portal`/`dropdown`/`collapse`/`modal`/`textInput`); delete client-local interaction maps | open | 11c | keyboard/focus parity + state persists across updates + z-order + visual |
| 14 | Retire god-object: host `EditorView`/editor surface as child; delete `SduiNativeState::paint` + `SduiLegacyLeaf`; shrink `masonry_sdui.rs` | open | 13 | compile-time absence of retired fns + full suite + visual |
...
- [ ] **(Step 13 — requires Step 11c)** Migrate transient surfaces: `overlay`/`portal`, then `dropdown`/`collapse`/`modal`/`textInput`
  - Acceptance Criteria:
...
- [ ] **(Step 14 — requires Step 13)** Host `EditorView` as a real child component and retire the `SduiNativeState` god-object
