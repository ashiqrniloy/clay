...
| 19 | Drop the `masonry_sdui.rs` test-infra `#![allow(dead_code)]`: `#[cfg(test)]`-gate or wire the observability/not-yet-wired API | done | 14 | `#![allow(dead_code)]` removed; 9 items `#[cfg(test)]`-gated, 5 dead helpers deleted, test-only imports + 2 shell re-exports gated; `cargo clippy --all-targets -- -D warnings` clean; lib 1146/editor 149/runtime 196 green |
...
- [x] **(Step 13f — requires Step 13e)** Hosted menu a11y parity; delete the legacy overlay/menu a11y branch
  - **Status: done (2026-07-30).** `PackageRegionWidget` gains a `menu_a11y: Option<MenuA11y>` payload (`prompt`, resolved item `label`s + `selected`, empty-state `status`) built once in `TransientPackageOverlay::from_menu_session` from the session and installed by `PackageOverlayHost::sync_overlays`. When `Some`, `PackageRegionWidget::accessibility` reports `Role::Menu` (label = sanitized prompt) with synthetic `Role::MenuItem` children (label = resolved accessibility label + a `" selected"` suffix on the active item) and a `Role::Status` child for the empty-state message, calling `node.set_children` so the generic reconciled subtree (`Group`/`ListItem`) is excluded — no double-report. When `None` (package-declared overlays, fixed panels) the default `Group` + children flow is unchanged. `EditorWidget::accessibility` now includes `overlay_host.id()` in its a11y children (replacing the legacy `append_accessibility_children` synthetic-node walk). Verified by `hosted_menu_overlay_exposes_menu_role_and_item_accessibility_labels` (asserts the `Menu`/`MenuItem` roles, the custom labels, and the `" selected"` suffix via a `RenderRoot` `TreeUpdate` after `WindowEvent::EnableAccessTree`).
  - **Deletions:** `append_accessibility_children`, `accessibility_entries`, `collect_active_menu_accessibility_entries`, `collect_package_accessibility_entries`, and the `SduiAccessibilityEntry` struct in `masonry_sdui.rs`; the `TransientMenuStatus` re-export from `shell/mod.rs`; the now-unused `Node`/`NodeId`/`AccessCtx`/`WidgetId` imports. The legacy test `active_menu_exposes_menu_role_and_item_accessibility_labels` is deleted (replaced by the hosted-parity test). `accessibility_nodes`/`collect_accessibility_nodes` (the SDUI-tree a11y walker) is **kept** — it is a separate test-only concern (SDUI tree, not overlays) used by the `sdui_accessibility_*` tests.
  - **Note on the second legacy test:** `accessibility_recovery_summary_uses_active_menu_prompt` (masonry_editor.rs) asserts `status_observation().recovery_summary` / `status_text` / `accessibility_label()` (the editor status line), not the overlay a11y tree, so it is unaffected and still passes.
  ...
  - Approach (options to confirm at execution time):
    - Chosen: `PackageRegionWidget` carries a `menu_a11y: Option<MenuA11y>` payload; `accessibility()` builds `Menu`/`MenuItem`/`Status` synthetic nodes + `set_children` (excluding the generic subtree) when `Some`, else the default `Group` + children flow. `MenuA11y` is built once in `from_menu_session` (resolved labels + selected + empty-state status) and installed by `sync_overlays`.
...
- [x] **(Step 19 — requires Step 14)** Drop the `masonry_sdui.rs` test-infra `#![allow(dead_code)]`
  - **Status: done (2026-07-31).** Removed the module-level `#![allow(dead_code)]` from `src/masonry_sdui.rs`. `#[cfg(test)]`-gated the 9 test-only observability/introspection items that remained; deleted the 5-item dead helper cluster (`package_ui`/`text_metrics`/`body_metrics`/`component_variant`/`component_metrics`) that had zero callers even in tests; gated the now-test-only imports + 2 shell re-exports. `cargo clippy --all-targets -- -D warnings` is clean with no module-level dead-code allow.
  ...
  - Acceptance Criteria:
    - Functional: The `#![allow(dead_code)]` is removed from `masonry_sdui.rs`; every item it masked is either `#[cfg(test)]`-gated (if test-only) or wired into production (if intended for production). No `dead_code` warnings under `cargo clippy --all-targets`. ✓ — `#[cfg(test)]`-gated: `SduiObservableListItem`, `SduiObservableSnapshot`, `typography_revision`, `apply_package_ui_update`, `package_ui_version`, `observable_snapshot`, `package_overlay_observations`, `collect_observable_snapshot`, `sdui_node_kind_name`. Deleted (dead even in tests — zero callers after Step 14 deleted the hit-test model): `package_ui` (accessor), `text_metrics`, `body_metrics`, `component_variant`, `component_metrics`. `theme_style` is production (`sidebar_geometry`) and stays ungated.
    - Code Quality: `cargo clippy --all-targets -- -D warnings` clean with no module-level `#![allow(dead_code)]` in `masonry_sdui.rs`. ✓ — clippy exit 0; `grep allow(.*dead_code src/masonry_sdui.rs` returns empty. Test-only imports (`BTreeSet`, `UiTextVariant`, `PackageUiComponentTree`, `PackageUiOverlayObservation`, `PackageUiPanelObservation`, `PackageUiRuntimeError`, `PackageUiRuntimeUpdate`) gated with `#[cfg(test)]` in `masonry_sdui.rs`; the 2 re-exports now used only by tests (`PackageUiOverlayObservation`, `PackageUiPanelObservation`) gated with `#[cfg(test)]` in `src/shell/mod.rs`. The `clippy::too_many_arguments` allow remains (production paint/layout helpers, unrelated).
    - No Regression: `cargo test --lib` / `--test editor` / `--test runtime` green; the observability-snapshot tests still pass (gated, not deleted). ✓ — lib 1146, editor 149, runtime 196 pass; protocol 139+1 / security 121+1 pre-existing failures unchanged.
  - Approach:
    - Items to `#[cfg(test)]`-gate (test-only observability): `observable_snapshot`, `collect_observable_snapshot`, `package_overlay_observations`, `SduiObservableSnapshot`, `SduiObservableListItem`, + the `sdui_observable_snapshot_*` tests that exercise them. ✓ — all 9 items gated.
    - Items to decide (test-only OR not-yet-wired): `apply_package_ui_update` (production uses `install_package_ui_snapshot`; `apply_package_ui_update` is called only from test modules in `masonry_editor.rs`), `package_ui_version`, `typography_revision`, `package_ui` (the accessor method, not the field — `panels_render_input` reads the field). Either `#[cfg(test)]`-gate them (if purely test-only) or wire the package-UI incremental path (if intended for production). Confirm intent per item before gating — some may be reserved for a future Clay JS observability facade (see the file's `// Internal test/agent observability surface only...` comment). ✓ — `apply_package_ui_update`/`package_ui_version`/`typography_revision` gated (test-only, no production consumer); `package_ui` accessor deleted (zero callers, even in tests — `panels_render_input` reads the field directly).
    - Cascading helpers: `body_metrics` / `text_metrics` / `component_variant` / `component_metrics` — gate or remove once their only callers are gated/removed; `theme_style` is production (sidebar_geometry) and stays. ✓ — all 4 deleted (dead cluster: their only callers were the deleted `collect_package_action_regions` hit-test model + each other; zero callers remained after Step 14).
  ...
  - Files to Edit:
    ...
    - `src/shell/mod.rs`: gate the 2 test-only re-exports (`PackageUiOverlayObservation`, `PackageUiPanelObservation`). ✓
  - References:
  ...
  - Test Cases to Write:
    - `cargo clippy --all-targets -- -D warnings` clean with the allow removed (the gate). ✓ — exit 0.
    - Existing `sdui_observable_snapshot_*` tests still compile + pass under `#[cfg(test)]`. ✓ — lib 1146 passed (includes the gated observability tests).
...
- **Step 14 — `masonry_sdui.rs` test-infra allow (resolved by Step 19).** The original "staged for runtime wiring" `#![allow(dead_code)]` was reframed in Step 14 to "test/agent observability surface". Step 19 removed it entirely: 9 test-only items (`SduiObservableListItem`/`SduiObservableSnapshot`/`typography_revision`/`apply_package_ui_update`/`package_ui_version`/`observable_snapshot`/`package_overlay_observations`/`collect_observable_snapshot`/`sdui_node_kind_name`) are now `#[cfg(test)]`-gated; the 5-item dead helper cluster (`package_ui`/`text_metrics`/`body_metrics`/`component_variant`/`component_metrics`) was deleted (zero callers after Step 14 deleted the hit-test model); test-only imports + 2 shell re-exports gated. No module-level dead-code allow remains in the SDUI surface.
...
- **Step 19 (done):** dropped the `masonry_sdui.rs` test-infra `#![allow(dead_code)]` — `#[cfg(test)]`-gated 9 test-only observability/introspection items, deleted the 5-item dead helper cluster, gated test-only imports + 2 shell re-exports. `cargo clippy --all-targets -- -D warnings` clean.
