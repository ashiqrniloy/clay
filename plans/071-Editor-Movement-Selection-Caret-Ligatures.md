...
- [x] 2. Review Clay UI catalog and plan primitive/component reuse before caret/ligature UI work ✓ (UI Catalog Review below)
  - Acceptance Criteria:
    - Functional: Confirm caret styling + ligatures reuse existing editor chrome primitives (`src/editor/surface.rs` `paint_caret`, `StyleRegistry`, `base.caret`, `FontProfile`) and the typography registry, not the package-facing component catalog; justify why no new `ComponentKind`/typed theme token is required for caret shape/blink. **Load the complete mandatory project-local UI skill stack** before reviewing OpenType features, caret styling, and accessibility/reduced-motion; apply that guidance to the caret-shape/blink and ligature design so the UI is aligned with current UI-craft conventions, not just the existing code.
  ...
  - Approach:
    - Documentation Reviewed:
      ...
      - **legacy selection workflow CLI** (run before UI design): the UI guidance current at execution time (legacy selection workflow routing skill) → the UI guidance current at execution time → the UI guidance current at execution time / `--category accessibility` → the UI guidance current at execution time. Primary pick: `jakubkrehel/better-typography` (in both `typography` + `accessibility`; its triggers explicitly cover OpenType features, `font-feature-settings`, variable fonts, and styling underlines/selection/placeholders/**carets**). Optional second: `mengto/minimalist-ui` (matches Clay's minimalist design language per `tokens.md`). Prefer 1, max 3. (Confirmed runnable: the UI guidance current at execution time installs `legacy selection workflow` on demand and prints the routing skill.)
    - Options Considered:
  ...
  - Test Cases to Write:
    ...
    - UI alignment: caret-shape/blink and ligature design choices are checked against the the UI guidance current at execution time-loaded `better-typography` guidance (caret styling, OpenType `font-feature-settings`, reduced-motion/no-blink accessibility option); deviations recorded with a reason.

  ### UI Catalog Review (Task 2 — completed 2026-07-31)

  **UI skills loaded (via the UI guidance current at execution time → `categories` → `list --category typography` → `get`):**
...
**Post-round fix (2026-08-04) — attribution stamp vs activation scope.** Enabling LSP packages + `clientSetCursorStyle` in `examples/init.js` exposed a latent leak: `op_clay_packages_load_package_by_specifier` stamps `current_package` so loadEntry registrations attribute to the package, and the stamp intentionally outlives the activation (init.js flows publish decorations/parse updates attributed to the loaded package afterwards). The task-19 gate keyed on stamp presence (`has_current_package`), so user-configuration editor calls after any `loadPackage` were treated as calls by the last loaded package and denied (`@clay/lsp-markdown lacks approved editor-control`). Fix: `ClayOpState.package_activation_depth` (nesting counter; `begin_evaluation` resets) entered by the loadPackage stamp, controlled package evaluations, and host-invoked parse/completion/analysis callbacks; exited by new trusted-only `op_clay_packages_end_package_activation` called from the `loadPackage` facade `finally`. `require_editor_control` and execute-command provenance now key on `in_package_activation()`, never on stamp presence. Also hardened the example: grants wrapped in a degrading `grantLanguageServer` helper (missing executable/root skips one server instead of sinking the whole configuration). New regression gate: `example_configuration_loads_cleanly_and_applies_effects` (src/server/mod.rs) loads `examples/init.js` through the real runtime-generation path and asserts load + typography effects — environment-independent because grants degrade. Op inventory rebaselined 77→78 (plan 061 Trusted-only 4→5). Gates: fmt/clippy clean; lib 1259, editor 155, protocol 140, runtime 197, security 122.

---

**Post-round fix (2026-08-04) — caret blink never reached the client.** Manual testing found `clientSetCursorStyle({ blink: "phase" })` was a no-op: the caret stayed solid. Root cause: task-6 shipped the three-layer resolution and the blink state machine, but the runtime-override layer had **no transport** — `op_clay_editor_set_cursor_style` only validated and returned a descriptor, never wrote `EditorSurface.caret_style_override`, and `CaretStyle::default_bar()` is `BlinkStyle::Solid`. Two stacked gaps made every blink/phase setting inert. Fix: the op now field-merges against the active manifest layer (`modes::merge_caret_style`), publishes on a new `caret_styles` broadcast + shared `caret_style_state` store, and the connection forwards it as `ServerMessage::CaretStyleOverride(Option<CaretStyle>)` (initial sync when an override is active, on generation change to clear stale overrides, and live). The client applies it in `apply_connection_event` and the event loop kicks the blink loop (`caret_animates` + `request_anim_frame`). Added validation bounds on `CaretStyle` (width/height/blink-period caps). New tests: `set_cursor_style_publishes_runtime_caret_override` (server publish) and `caret_style_override_event_applies_and_reports_change` (client apply). Protocol 8→9. Sequence tests updated to skip the new handshake message; initial-sync send is conditional on `Some` to avoid noise. Gates: fmt/clippy clean; lib 1261, editor 155, protocol 140, runtime 197, security 122.
