...
`frontend/src/command-centre/CommandPalette.tsx` projects the server-owned command and path sessions as the composer field's own menu (plan 124: `TransientMenuOrigin::CommandPalette`, `list.default.row.*`/`seg.default.*` over `commandCentre.*`, veiled by `modal.scrim`), while `frontend/src/command-centre/CommandCentre.tsx` keeps the window-centered `Centered` hosts (agent picker, package UI dialogs) through the same surface (React Aria `Modal`/`Dialog`, `TextField`, `ListBox`; results capped at 256). `frontend/src/settings/SettingsPanel.tsx` is the compiled trusted presentation for `@clay/settings`: existing primitives only, host-resolved tokens, versioned `settings.*` intents.
...
| `paint_scrim` | internal | Dim behind the window-centered Command Centre sheet (Phase 24.4) and, since plan 124, behind the composer's `/` and `@` menus — over the working area only, never over the agent lane (the field that *is* the palette's query stays interactive) | `surface.scrim`, `opacity.scrim` | `dialog` backdrop (Clay-internal; no package-facing surface) |
...
| Command palette | implemented | Clay-owned Command Centre surface | Phase 24.2/24.4, re-anchored by plan 124: `controlCenter.open` opens the composer's `/` palette (`CommandPalette` origin) — generation-stamped live command catalogue with per-row scope and chords, the field as its query, the sheet as wide as the box it answers to with the composer's own controls, retained scroll; the window-centered sheet remains for the hosts whose origin is `Centered` (agent picker, package dialogs). Clay-internal: packages cannot open or drive it. No new recipe key: the sheet is `commandCentre.*` over `list.default.row.*` / `seg.default.*`, veiled by `modal.scrim` |
