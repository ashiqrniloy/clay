...
**Surface origins**: `TransientMenuOrigin` selects the host-owned anchor and focus policy. `CommandPalette` is the bottom-anchored built-in menu — since plan 125 the *only* transient selection surface, hosting the command catalogue, the Path Browser, and every agent-picker stage (its bounded `mode` field selects what the sheet renders); `ContextMenu`/`MenuBar` map to pointer/main package-compatible geometry. Plan 087 adds the Clay-owned `Completion` origin (caret/IME-adjacent modeless picker) and plan 125 retires `Centered` (window-level Command Centre/Path Browser, then agent-picker host): the value still decodes for old frames, no producer emits it, and no internal anchor vocabulary carries a centered option any more. Both remaining internal origins stay internal: packages keep only `working-area`, `active-pane`, `main`, `pointer`; `TransientPackageOverlay::from_menu_session` maps each internal origin without exposing it.
...
**Composer palette projection (Plan 124 / Plan 125)**: one Clay-owned composer-attached host for every session — the sheet 6 logical pixels above the field, exactly as wide as the box, veiled by the `modal.scrim` recipe over the working area through the inspector rail's full height (the lane stays above the veil because it holds the query), `commandCentre.*` over `list.default.row.*`/`seg.default.*`, the halo instead of a drop shadow, retained scrollable result list capped at 256 rows. `mode` selects the rendering: `catalogue`/`path` echo the `/`-led query, the picker stages render stage prompts and a mode-aware foot, and `secret` is the one stage with a shielded `type="password"` field inside the sheet (composer field disabled, value masked in snapshots and delivered only through the host credential path). Packages may register commands in the catalogue but cannot open, drive, configure, intercept, veil, or shield this surface. The retired **Centered Command Centre projection** (Phase 24.4 / Plan 087) is recorded in `DESIGN.md` §14: one window-level host, one token-driven scrim, `dimension.overlay.centered.width` (640 logical-pixel default), modal Dialog/Menu/Status accessibility — no live producer, deleted in plan 125.
...
- `@clay/settings` demonstrates the composition contract (`panel` + `scroll` + existing controls). `table` remains the only reserved package kind; package overlays stay limited to `working-area`, `active-pane`, `main`, `pointer`; `completion` and `centered` are Clay-internal origins (plan 125 retires `centered`: it decodes for old frames only and no anchor vocabulary carries it any more).
...
`frontend/src/command-centre/CommandPalette.tsx` projects every server-owned session — the command catalogue, the Path Browser, and the agent picker's list/provider-setup/auth-method/secret/URL/OAuth stages — as the composer field's own menu (plan 124/125: `TransientMenuOrigin::CommandPalette`, `list.default.row.*`/`seg.default.*` over `commandCentre.*`, veiled by `modal.scrim`, halo shadow, stage prompts and a mode-aware foot, shielded in-sheet field for `secret`, results capped at 256; the centered `CommandCentre.tsx` host is deleted). `frontend/src/settings/SettingsPanel.tsx` is the compiled trusted presentation for `@clay/settings`: existing primitives only, host-resolved tokens, versioned `settings.*` intents.
...
| Agent lane | internal | `frontend/src/shell/AgentLane.tsx` | Plan 124 Clay-owned persistent bottom chrome row of the **view pane** (never a package slot, kind, origin, or anchor): optional approval strip + composer box (prompt field plus the tab's agent-type/model/effort toolbar) + hint row + session-environment foot, shared by the Workspace and Agent views of the tab, toggled by `shell.toggleAgentLane` with per-tab visibility persisted by the client. Plan 125 confined it: the sidebar and the inspector rail run the full working-area height and the lane stops at the rail's inner edge (full width only with the rail collapsed) |
...
| Transient menu | internal | `src/shell/transient_menu.rs` | Bounded prompt/item list scored by the shared fuzzy matcher (`src/shell/fuzzy.rs`), focus policy, package provenance; server-owned Control Center/Path Browser sessions plus Plan 087/Phase 24.4 hosts. Plan 124 adds the bottom-anchored `CommandPalette` origin: the composer's `/` palette and its path mode, drawn as the field's own menu (the field holds the query, so the session filters and re-selects); plan 125 makes it the only transient selection surface, adds the bounded `mode` field (`catalogue`, `path`, `picker`, `secret`, `url`, `oauth`), the stage flows with `Esc`/`Alt+←` stage-back and `Alt+↵` secondary activation, and the shielded `secret` field |
...
| Transient package overlays | internal | `src/shell/package_ui.rs` | Package-declared overlays using only `working-area`, `active-pane`, `main`, `pointer`; the completion host and the composer palette are Clay-internal, and `PackageOverlayAnchor` has no centered option (plan 125 retired it, mapping the old wire value to the bottom work-area anchor) |
...
| `paint_scrim` | internal | Dim behind the composer's `/` palette and `@` mentions menu (plan 124) — over the working area and the inspector rail's full height, never over the agent lane (the field that *is* the palette's query stays interactive). Retired with the centered sheet (plan 125) | `surface.scrim`, `opacity.scrim` | `dialog` backdrop (Clay-internal; no package-facing surface) |
...
- **Typography boundary:** host CSS names a font *role*, never a stack — every `font-family` is `inherit` or `var(--clay-font-ui|monospace|proportional)`, the tokens the user's typography profile feeds (`frontend/src/theme/adapter.ts`). A concrete stack (`ui-monospace, monospace`) or a role that exists nowhere silently drops the declaration, so the surface renders in the wrong font *and* ignores the profile; both fail `checkFontRoleDiscipline` (`frontend/src/test/design-system-consumption.test.ts`). Same gate, same rule for colour: nothing under `frontend/src` outside `tokens.css` may carry a colour literal (`checkNoRawColors`) — design language §8's "never a concrete family or size" and §11's role/recipe ownership, enforced instead of trusted.
...
| Command palette | implemented | Clay-owned composer palette (the only transient selection surface) | Phase 24.2/24.4, re-anchored by plan 124 and completed by plan 125: `controlCenter.open` opens the composer's `/` palette (`CommandPalette` origin) — generation-stamped live command catalogue with per-row scope and chords, the field as its query, the sheet as wide as the box it answers to, retained scroll, halo instead of a drop shadow; the same sheet renders the Path Browser and every agent-picker stage behind the bounded `mode` field, including the shielded `secret` stage. The window-centered sheet is retired (no live producer, projection deleted). Clay-internal: packages cannot open or drive it. Recipe keys are unchanged: `commandCentre.*` over `list.default.row.*` / `seg.default.*`, veiled by `modal.scrim` |
