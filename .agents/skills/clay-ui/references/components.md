...
| Command palette | implemented | Command Centre surface | Phase 20.5: `TransientMenuOrigin::CommandPalette` (default); `Modal` focus policy + `Bottom` anchor. Phase 24.1/24.2: opens via the `controlCenter.open` command (default `Ctrl+Shift+P`), lists the generation-stamped live command catalogue (built-ins + `shell.client*` + package commands) with effective keybindings/provenance detail, and activates through the shared server execution path or the `ShellClientCommandRequest` bridge |
