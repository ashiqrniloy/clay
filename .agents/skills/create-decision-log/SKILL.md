...
6. **Update reusable project guidance when available**
   - If the project has `.agents/skills/clay-execution/`, extract stable reusable planning guidance from the decision (not the full log text) and add or update a concise file under `.agents/skills/clay-execution/references/` (one pattern per file, kebab-case names). If the decision is too narrow to be reusable, add nothing and say so.
   ...
   - The clay-execution router SKILL.md documents this workflow in full.
