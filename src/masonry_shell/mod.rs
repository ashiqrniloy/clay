...
use crate::protocol::{ActiveTypography, ClientId, FontRole};
use crate::shell::{
    ...
    SlotDragState, SplitChild, SplitDragState, SplitOrientation, SplitRatio,
...
// Plan 090 task 6: extracted shell tab/window layer + accessibility builder.
mod accessibility;
mod window_tabs;

#[cfg(test)]
pub(crate) use self::accessibility::ANNOUNCEMENT_MAX_CHARS;
...
pub(crate) use self::window_tabs::{
    SHELL_CLIENT_COMMAND_CATALOGUE, TAB_BAR_CARD_GAP, TAB_BAR_CARD_MIN_WIDTH, TAB_BAR_CARD_PADDING,
    TAB_BAR_CARD_WIDTH, TAB_BAR_CLOSE_SIZE, TAB_BAR_HEIGHT, TAB_BAR_NEW_TAB_GAP,
    TAB_BAR_NEW_TAB_SIZE, TAB_BAR_SCROLL_STEP, TabBarGeometry, TabCardGeometry,
};
...
pub struct ClayShellWidget {
    ...
    tab_bar_scroll: f64,
    /// Shell text paint and tab-bar geometry use the active tab's cached UI
    /// typography profile; updates invalidate the window layout once.
...
impl ClayShellWidget {
    pub fn single_editor(client_id: ClientId, editor: EditorWidget) -> Self {
        ...
        Self::from_chrome(
            client_id,
            TabChrome::single_editor(editor, true),
            ui_theme,
            typography,
        )
    }
    ...
    pub fn install_tab(&mut self, ctx: &mut MutateCtx<'_>, client_id: ClientId, chrome: TabChrome) {
        ...
        let tab_typography = chrome.typography.clone();
        self.tabs.insert(client_id, chrome);
        if first {
            self.active_tab = client_id;
            self.typography = tab_typography;
        }
    ...
    pub fn set_active_tab(&mut self, ctx: &mut MutateCtx<'_>, client_id: ClientId) -> bool {
        ...
        self.active_tab = client_id;
        self.typography = self.active().typography.clone();
    ...
    pub fn remove_tab(&mut self, ctx: &mut MutateCtx<'_>, client_id: ClientId) {
        ...
        let next_active = self
            ...
            .keys()
            .copied()
            .find(|candidate| *candidate != client_id);
        let repair_focus = was_active && ctx.has_focus_target();
        if let Some(next) = next_active.filter(|_| was_active) {
        ...
        // The survivor is still stashed until the next shell layout pass;
        // clear now, then the driver focuses it after layout.
        if repair_focus {
            ctx.resign_focus();
        }
    ...
    /// Tab-bar height follows the user-owned UI typography while retaining a
    /// compact default and never consuming more logical height than exists.
    fn tab_bar_height(&self, available_height: f64) -> f64 {
        let metrics = self
            .typography
            .ui_text_metrics(FontRole::Ui, UiTextVariant::Status);
        TAB_BAR_HEIGHT
            .max(metrics.status_height())
    ...
    pub(crate) fn tab_bar_geometry(&self, size: Size) -> Option<TabBarGeometry> {
        ...
        let bar = Rect::new(
            ...
            0.0,
            size.width.max(0.0),
            self.tab_bar_height(size.height),
        );
        let card_gap = TAB_BAR_CARD_GAP.min(bar.height() / 2.0).max(0.0);
        let new_tab_gap = TAB_BAR_NEW_TAB_GAP.min(bar.width() / 2.0).max(0.0);
        let preferred_new_tab_size =
            TAB_BAR_NEW_TAB_SIZE.max(f64::from(metrics.font_size) + card_gap * 2.0);
        let new_tab_size = preferred_new_tab_size
            .min(bar.height().max(0.0))
            .min((bar.width() - new_tab_gap).max(0.0));
        let new_tab_rect = Rect::new(
            (bar.x1 - new_tab_size - new_tab_gap).max(bar.x0),
            bar.y0 + (bar.height() - new_tab_size) / 2.0,
            (bar.x1 - new_tab_gap).max(bar.x0),
            bar.y0 + (bar.height() + new_tab_size) / 2.0,
        );
        ...
        let mut x = bar.x0 + card_gap;
        for _ in &self.tab_cards {
            let width = TAB_BAR_CARD_WIDTH
                .min((new_tab_rect.x0 - x - card_gap).max(TAB_BAR_CARD_MIN_WIDTH));
            let rect = Rect::new(
                x,
                bar.y0 + card_gap,
                x + width,
                (bar.y1 - card_gap).max(bar.y0 + card_gap),
            );
            let card_padding = TAB_BAR_CARD_PADDING.min(rect.height() / 2.0).max(0.0);
            let close_size = TAB_BAR_CLOSE_SIZE
                .max(f64::from(metrics.font_size))
                .min(rect.height())
                .max(0.0);
            cards.push(TabCardGeometry {
                rect,
                label_rect: Rect::new(
                    rect.x0 + card_padding,
                    rect.y0,
                    (rect.x1 - card_padding - close_size - card_padding)
                        .max(rect.x0 + card_padding),
                    rect.y1,
                ),
                close_rect: Rect::new(
                    rect.x1 - card_padding - close_size,
                    rect.y0 + (rect.height() - close_size) / 2.0,
                    rect.x1 - card_padding,
                    rect.y0 + (rect.height() - close_size) / 2.0 + close_size,
                ),
            });
            x = rect.x1 + card_gap;
        }
        // Last card's right edge can reach the "+" slot boundary.
        let scroll_max = (x - card_gap - new_tab_rect.x0).max(0.0);
    ...
    fn apply_tree_change(
        ...
        if let Some(new_tree) = new_tree {
            ...
            // Move keyboard/AT focus onto a surviving pane before the next
            // AccessKit update. Doing this after detach avoids advertising a
            // removed host (accesskit_consumer: "Focused ID is not in the node list").
            if ctx.has_focus_target() {
                if let Some(target) = self.active_pane_target() {
    ...
    /// Install a newer user-owned typography snapshot for one tab's shell
    /// chrome. The active tab also drives the window-level tab bar; inactive
    /// tabs retain their registry until activation.
    #[doc(hidden)]
    pub fn set_active_typography(
        &mut self,
        client_id: ClientId,
        ...
        let Ok(changed) = tab.typography.install(typography) else {
            return false;
        };
        if changed && client_id == self.active_tab {
            self.typography = tab.typography.clone();
        }
        changed
    }
...
impl Widget for ClayShellWidget {
    ...
    fn accessibility(
        ...
        if self.tab_cards.len() >= 2
            && let Some(geometry) = self.tab_bar_geometry(accessibility::node_window_size(node))
        {
            ...
            list.set_bounds(accessibility::accesskit_rect(geometry.bar));
            ...
            for (card, card_geometry) in self.tab_cards.iter().zip(&geometry.cards) {
                ...
                tab.set_bounds(accessibility::accesskit_rect(card_geometry.rect));
...
mod tests {
    ...
    use masonry::core::{Ime, PointerButtonEvent, TextEvent, WindowEvent};
    ...
    fn render_root_options() -> RenderRootOptions {
        render_root_options_with_scale(1.0)
    }

    fn render_root_options_with_scale(scale_factor: f64) -> RenderRootOptions {
        RenderRootOptions {
    ...
    #[test]
    fn top_level_frame_focus_action_does_not_dispatch_to_a_widget() {
        let (mut render_root, editor_widget_id) = render_root_for_shell(EditorWidget::default());
        let update = access_tree(&mut render_root);
        let frame_id = update.tree.expect("access tree has a top-level frame").root;
        assert!(render_root.focus_on(Some(editor_widget_id)));
        assert_eq!(render_root.focused_widget(), Some(editor_widget_id));

        render_root.handle_access_event(masonry::accesskit::ActionRequest {
            action: masonry::accesskit::Action::Focus,
            target: frame_id,
            data: None,
        });
    ...
    fn pane_close_removes_routing_target_and_reconciles_host() {
        ...
        let _ = render_root.redraw();
        assert!(render_root.focus_on(Some(view2_id)));
        let mut consumer = consumer_tree(&mut render_root);

        // Close pane 2 (the active pane); focus must move before its host is
        // detached so the consumer never receives the removed view id.
        dispatch_shell_command(&mut render_root, shell_id, ShellClientCommand::ClosePane);
        consumer_update(&mut consumer, &mut render_root);
        assert_eq!(render_root.focused_widget(), Some(editor_widget_id));
    ...
    #[test]
    fn dirty_pane_close_rejection_and_discarded_removal_keep_focus_consumer_safe() {
        let (queue, _receiver) = ClientEditQueue::bounded(16);
        let layout = WorkingAreaLayout::with_pane_tree(two_pane_tree(), PaneId(1)).unwrap();
        let shell = ClayShellWidget::single_editor_with_layout(
            EditorWidget::with_initial_state(initial_state(
        ...
        render_root.edit_widget(editor_widget_id, |mut widget| {
            ...
            assert!(
                editor
                    .widget
                    .apply_connection_event(ClientConnectionEvent::DocumentSaved {
                        document_id: 7,
                        version: 3,
                        dirty: true,
                    })
        ...
        // The dirty-close gate rejects removal and leaves focus on the live
        // editor target.
        render_root.edit_widget(editor_widget_id, |mut widget| {
            let editor = widget.try_downcast::<EditorWidget>().expect("editor");
            assert!(!editor.widget.guard_pane_close());
            editor.widget.take_pending_menu();
        });
        consumer_update(&mut consumer, &mut render_root);
        assert_eq!(render_root.focused_widget(), Some(editor_widget_id));

        // Simulate the discard choice, then remove the formerly dirty pane.
        render_root.edit_widget(editor_widget_id, |mut widget| {
            let editor = widget.try_downcast::<EditorWidget>().expect("editor");
        ...
        dispatch_shell_command(&mut render_root, shell_id, ShellClientCommand::ClosePane);
        consumer_update(&mut consumer, &mut render_root);
        assert_consumer_focus_live(&consumer);
        assert_ne!(render_root.focused_widget(), Some(editor_widget_id));
    }

    #[test]
    fn dirty_focused_pane_menu_and_discard_keep_consumer_focus_live() {
        let (queue, _receiver) = ClientEditQueue::bounded(16);
        let layout = WorkingAreaLayout::with_pane_tree(two_pane_tree(), PaneId(1)).unwrap();
        let shell = ClayShellWidget::single_editor_with_layout(
            EditorWidget::with_initial_state(initial_state(
        ...
        let view2_id = mount_document_view(&mut render_root, shell_id, PaneId(2), view2);
        render_root.edit_widget(view2_id, |mut widget| {
            let mut view = widget.try_downcast::<PaneDocumentView>().expect("view");
            assert!(
                view.widget
                    .apply_connection_event(document_opened(4, 1, "a.txt", "hello",))
            );
            let _ = view.widget.paste_provided_clipboard_text("x");
            assert!(view.widget.is_dirty());
        ...
        // Driver dirty-close gate: reject removal and project the conflict menu.
        let menu = render_root.edit_widget(view2_id, |mut widget| {
            let view = widget.try_downcast::<PaneDocumentView>().expect("view");
            assert!(!view.widget.guard_pane_close());
            view.widget.take_pending_menu()
        });
        let menu = menu.expect("dirty close pushes a menu").expect("session");
        render_root.edit_widget(editor_widget_id, |mut widget| {
            let mut editor = widget.try_downcast::<EditorWidget>().expect("editor");
            editor.widget.set_active_menu(Some(menu));
            editor.widget.sync_overlays(&mut editor.ctx);
            editor.ctx.request_accessibility_update();
        });
        consumer_update(&mut consumer, &mut render_root);
        assert_consumer_focus_live(&consumer);
        assert_eq!(render_root.focused_widget(), Some(view2_id));

        // FileOperationFailed { DirtyDocument } is the live crash log path.
        render_root.edit_widget(view2_id, |mut widget| {
            let view = widget.try_downcast::<PaneDocumentView>().expect("view");
            assert!(view.widget.apply_connection_event(
                ClientConnectionEvent::FileOperationFailed {
                    code: crate::protocol::FileErrorCode::DirtyDocument,
                    message: "Document has unsaved edits".to_string(),
        ...
        // Discard, then close the focused dirty pane.
        render_root.edit_widget(editor_widget_id, |mut widget| {
            let mut editor = widget.try_downcast::<EditorWidget>().expect("editor");
    ...
    #[test]
    fn tab_bar_height_tracks_user_ui_typography() {
        let mut state = initial_state(DocumentAccess::Editable { lease_id: 1 }, 3);
        state.active_typography.revision = 1;
        state.active_typography.ui.size = 24.0;
        let shell = ClayShellWidget::single_editor(0, EditorWidget::with_initial_state(state));
        let shell_new = NewWidget::new(shell);
        let shell_id = shell_new.id();
        let mut render_root = RenderRoot::new(shell_new, |_| {}, render_root_options());
        ...
        render_root.edit_widget(shell_id, |mut widget| {
            ...
            let geometry = shell
                .widget
                .tab_bar_geometry(Size::new(900.0, 600.0))
                .expect("tab bar visible");
            assert!(geometry.bar.height() > TAB_BAR_HEIGHT);
            assert_eq!(
                shell.widget.pane_host_rects(Size::new(900.0, 600.0))[0]
                    .1
                    .y0,
                geometry.bar.y1
            );
    ...
    #[test]
    fn live_typography_update_reflows_tab_bar_without_duplicate_churn() {
        let (mut render_root, shell_id, _) = tab_bar_two_card_root();
        let default = crate::protocol::ActiveTypography::default();
        let typography = crate::protocol::ActiveTypography {
            revision: 1,
        ...
        let changed = render_root.edit_widget(shell_id, |mut widget| {
            let shell = widget.try_downcast::<ClayShellWidget>().expect("shell");
            shell.widget.set_active_typography(2, typography.clone())
        });
        assert!(changed);
        let duplicate = render_root.edit_widget(shell_id, |mut widget| {
            let shell = widget.try_downcast::<ClayShellWidget>().expect("shell");
            shell.widget.set_active_typography(2, typography)
        });
        assert!(!duplicate);
        let _ = render_root.redraw();
        let height = render_root.edit_widget(shell_id, |mut widget| {
            let shell = widget.try_downcast::<ClayShellWidget>().expect("shell");
            shell
                .widget
                .tab_bar_geometry(Size::new(900.0, 600.0))
        ...
        assert!(height > TAB_BAR_HEIGHT);
    }

    #[test]
    fn high_dpi_layout_uses_logical_window_bounds() {
        let shell = ClayShellWidget::single_editor(0, EditorWidget::default());
        let shell_new = NewWidget::new(shell);
        let shell_id = shell_new.id();
        let mut render_root =
            RenderRoot::new(shell_new, |_| {}, render_root_options_with_scale(2.0));
        render_root.edit_widget(shell_id, |mut widget| {
            let mut shell = widget.try_downcast::<ClayShellWidget>().expect("shell");
            shell.widget.set_tab_cards(
                &mut shell.ctx,
        ...
        let _ = render_root.redraw();
        let logical_size = render_root
            .get_widget(shell_id)
            .expect("shell remains mounted")
            .ctx()
            .size();
        assert_eq!(logical_size, Size::new(450.0, 300.0));
        let bounds = Rect::new(0.0, 0.0, logical_size.width, logical_size.height);
        render_root.edit_widget(shell_id, |mut widget| {
            let shell = widget.try_downcast::<ClayShellWidget>().expect("shell");
            let geometry = shell
                .widget
                .tab_bar_geometry(logical_size)
                .expect("tab bar visible");
            assert!(bounds.contains(Point::new(geometry.bar.x0, geometry.bar.y0)));
            assert!(geometry.bar.x1 <= bounds.x1 && geometry.bar.y1 <= bounds.y1);
            for (_, rect) in shell.widget.pane_host_rects(logical_size) {
    ...
    #[test]
    fn rescale_event_recomputes_logical_bounds_from_physical_size() {
        let shell = ClayShellWidget::single_editor(0, EditorWidget::default());
        let shell_new = NewWidget::new(shell);
        let shell_id = shell_new.id();
        let mut render_root = RenderRoot::new(shell_new, |_| {}, render_root_options());

        render_root.handle_window_event(WindowEvent::Rescale(2.0));
        render_root.handle_window_event(WindowEvent::Resize(PhysicalSize::new(1800, 1200)));
        let _ = render_root.redraw();
        ...
        let logical_size = render_root
            ...
            .ctx()
            .size();
        assert_eq!(logical_size, Size::new(900.0, 600.0));
    }
    ...
    fn remove_tab_uninstalls_hosts_and_switches_to_first_remaining() {
        let (mut render_root, shell_id, _) = tab_bar_two_card_root();
        let active_target = render_root.edit_widget(shell_id, |mut widget| {
            let shell = widget.try_downcast::<ClayShellWidget>().expect("shell");
            shell
                .widget
                .active_pane_target_for(2)
                .expect("active target")
        });
        assert!(render_root.focus_on(Some(active_target)));
        let mut consumer = consumer_tree(&mut render_root);

        // Removing the active tab (2) leaves tab 0 active with its single
        // host still registered and repairs focus before detaching tab 2.
        render_root.edit_widget(shell_id, |mut widget| {
            let mut shell = widget.try_downcast::<ClayShellWidget>().expect("shell");
            shell.widget.remove_tab(&mut shell.ctx, 2);
        });
        consumer_update(&mut consumer, &mut render_root);
        assert_eq!(render_root.focused_widget(), None);
        render_root.edit_widget(shell_id, |mut widget| {
            let shell = widget.try_downcast::<ClayShellWidget>().expect("shell");
            assert_eq!(
                shell.widget.working_area_layout().active_pane_id(),
    ...
    fn assert_consumer_focus_live(tree: &accesskit_consumer::Tree) {
        let focus = tree.state().focus_id_in_tree();
        assert!(
            tree.state().has_node(focus),
            "focused {focus:?} missing from accesskit_consumer tree"
        );
    }
