...
impl Driver {
    ...
    fn accept_registry_snapshot(&mut self, snapshot: &TabRegistrySnapshot) -> bool {
...
mod tests {
    ...
    fn registry_snap(
        revision: u64,
        entry: Option<&clay::protocol::TabEntry>,
    ) -> TabRegistrySnapshot {
        TabRegistrySnapshot {
    ...
    fn accept_registry_snapshot_rejects_stale_relays_and_resets_on_restart() {
        ...
        // even at a lower revision (in-place reconnects keep this driver).
        // ponytail: a restarted server whose registry re-fills before this
        // window's first reconnect lands resyncs at the next mutation (every
        // tab action bumps the revision); tab-bar cosmetics only.
