...
fn lane_heap_limits_stay_within_the_configured_budget() {
    ...
    assert_eq!(super::RuntimeLane::ALL.len(), JS_RUNTIME_LANES_PER_DOMAIN);
    assert_eq!(
        super::lane_heap_limit_bytes(JS_RUNTIME_HEAP_LIMIT_BYTES, RuntimeLane::General),
        JS_RUNTIME_HEAP_LIMIT_BYTES
    );
    assert_eq!(
        super::lane_heap_limit_bytes(JS_RUNTIME_HEAP_LIMIT_BYTES, RuntimeLane::Latency),
        JS_RUNTIME_LATENCY_LANE_HEAP_LIMIT_BYTES
    );
    const { assert!(JS_RUNTIME_LATENCY_LANE_HEAP_LIMIT_BYTES < JS_RUNTIME_HEAP_LIMIT_BYTES) };
    assert_eq!(
        super::lane_heap_limit_bytes(4 * 1024, RuntimeLane::Latency),
        4 * 1024,
