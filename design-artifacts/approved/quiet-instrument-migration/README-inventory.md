...
**Slots below are the package contract, not the prose.** They are read from
`packages/design-neobrutal/package.json` (50 families / 142 keys, minus chat).
The first draft of this table took its slots from
`docs/development/ui-design-system-recipe-matrix.md`, which documents a wider
vocabulary than any shipped package declares — see finding 6. The package key
set is the contract; the prose is a wish list until a surface consumes it.
...
| `list` (slot `row` ×rest/hover/active/focus/selected; no `root`) | `components/controls.module.css` | 23 refs (5 recipes) | transparent root, no border; row r8; hover `surface.hover`; selected accent @0.15 + 2px inset accent bar on navigation lists; `rowDetail` `text.muted` caption | catalog + every list in the app |
...
| `fileBrowser` (slot `root`) | `packages/package-workspace.module.css` (SDUI tree: `src/shell/file_browser.rs` → `sdui/renderer`) | 4 refs | item selected accent @0.15 + 2px inset bar; counts right-aligned mono `text.muted` | `workspace.html`, `package-workspace.html` |
| `settingsPanel` (slots `panel`, `heading`, `actions`) | `settings/settings-panel.module.css` | 11 refs | panel veil r12; heading `typography.section`; fields = catalog components, no bespoke chrome | `settings.html` |
| agent settings surface (not a recipe family — theme + host CSS) | `agent-settings/agent-settings.module.css` | **0 refs**, 14 `--clay-*` theme vars | must be rebuilt from catalog components (task 20) | `agent-settings.html` |
...
| Settings | `settings/SettingsPanel.tsx`, `settings-panel.module.css` | catalog components only; no bespoke panel chrome | `settings.html` |
| Agent settings | `agent-settings/AgentSettingsPanel.tsx`, `agent-settings.module.css` | same rule; typed `designTokens`-driven theme choices | `agent-settings.html` |
| Command centre | `command-centre/CommandCentre.tsx`, `command-centre.module.css` | the only global launcher; every action shows its key | `command-centre.html` |
...
| `settings.html` (task 4) | settings panel fields (text inputs, selects, toggles, groups) | invalid field and the not-yet-valid action bar (**dom**); disabled control (**catalog**) | 4 | both |
| `agent-settings.html` (task 4) | the agent's Settings tab: delivered files, sizes, provenance badges | delivered, nothing delivered, unreadable (**scene** ×3) | 4 | both |
| `command-centre.html` (task 4) | palette dialog, input, result groups, empty state, status row | results (**dom**); no-results (**behaviour**); querying and keyboard-selected row (**catalog**) | 4 | both |
| `package-workspace.html` (task 4) | SDUI panel/flex/stack/overlay surfaces, file browser, status bar | selected row and the failed-verification card (**dom**); hover (**catalog**) | 4 | both |
| `overlays.html` (task 4) | modal + scrim, dropdown + popover, menu, tooltip, toast | full veils and reduced transparency (**scene** ×2); dismissal and focus trapping are component behaviour (React Aria), not staged | 4 | both |
