/* ============================================================================
   Clay design proposal — shared interaction layer.

   One behavior layer, two design languages. Every screen HTML (neobrutal and
   reimagined) loads this file; the geometry/materials come from
   ds-neobrutal.css / ds-quiet.css, the color contract from theme.css.
...
(function () {
  ...
  function applyPrefs() {
    applyTheme(prefs.theme, { silent: true });
  ...
  function init() {
    ...
    window.addEventListener('beforeunload', function () {
      var current = document.querySelector('[data-app]');
      if (!current) return;
      save({
        sidebar: current.dataset.sidebar === 'collapsed' ? 'collapsed' : 'expanded',
        inspector: current.dataset.inspector === 'collapsed' ? 'collapsed' : 'expanded'
      });
    });
  }
