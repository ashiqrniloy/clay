...
| `approved/composer-palette-stages/` | The approved composer palette stage flows + halo set (2026-09-18): the plan-125 change — one bottom-anchored sheet for the catalogue and every picker stage (provider, sign-in method, shielded secret, base URL, OAuth device code, model, session) with the stage prompt, the stage-aware foot, the shielded secret field, the halo replacing the palette's and the `@` mentions menu's drop shadow, and the lane's width rule (the middle pane only; the working area's whole width when both rails are hidden; the sheet the lane's inner width) — frozen with per-file hashes and the approval record (including the approving statement verbatim) in its `README.md`. Binding for plan 125 implementation; the 18 scenes, coverage, catalog mapping and seven decisions stay in `README-prototype.md`. |
| `prototypes/composer-palette-stages/` | The plan-125 exploration (approved 2026-09-18; working copy): `palette-stages.html` + `palette-stages.css` over a copy of the migration kit. Eighteen scenes: the lane (rest, both rails, both rails hidden), the catalogue (query, empty, no match, scope chips, chord chips), the picker stages (provider, auth method, shielded secret, base URL, OAuth device code, a long model list, sessions), the stage-back trail, the `@` mentions menu and the halo comparison board — with the lane-containment rule, the stage model and the halo values in its `README.md`. |
| `screenshots/composer-palette-stages/` | The plan-125 capture: 30 PNGs (30 of the 144 asserted matrix runs — every scene at gruvbox-material-dark/wide, the halo board and the secret stage at four themes, the rails in a light theme, and the narrow frames that change geometry) plus `report.json` (all 144 asserted runs + the 15-step keyboard walk), written by `tools/capture-palette-stages.mjs`. |
| `tools/capture-palette-stages.mjs` | Verifies the plan-125 prototype and the frozen set (18 scenes × 4 themes × 2 widths = 144 runs) — theme/scene really applied, the lane is the **view pane's** width and never sits over a rail (rails at full height, the working area's whole width when none shows), the sheet is the composer box's width and the lane's inner width 6px above it, capped and scrolling, the stage prompt vs the catalogue head, scope chips only for the catalogue, the mode-aware foot (`↵` verb, `Esc`/`Alt+←` back, `Alt+↵` delete, the live result count), the secret stage a masked field whose value is bullets only and never the composer draft, the halo on the sheet and the mentions menu with the veil the only refraction, no console errors/remote requests/overflow — then walks the keyboard path (`/`, typing, arrows, `Esc` + reopen, `@`, `Ctrl+X Ctrl+O`, a depth-3 back trail, `Alt+↵`, a scope chip), writing `screenshots/composer-palette-stages/`. Exits non-zero on any failure. `--page=<file>` points the same assertions at the frozen set (`approved/composer-palette-stages/palette-stages.html`), `--filter=<regex>` narrows the matrix, `--no-shots` asserts only, `--out=<dir>` moves the captures. |
...
| `tools/capture-sidebar.mjs` | Drives the workspace sidebar over CDP (`?fixture=workspace-sidebar`, which now renders through `WorkspaceView` + `WorkspacePanes` — the app's own composition) and asserts the approved head's claims — the filter field with its `/` chip, local filtering of the delivered rows, the live match count, the foot hints, the 244px/224px token-sized region, zero horizontal overflow — **plus the plan-126 rail claims** at 1500/1024/900: the sidebar is the working area's left rail (bottom == the working area's bottom, so it runs through the lane's row), the lane starts at the rail's inner edge and ends at the inspector's, and below 1000px both rails are fixed drawers with the lane at the pane's full width. Writes `screenshots/quiet-instrument-sidebar/` (`CLAY_REVIEW_OUT` overrides). |
