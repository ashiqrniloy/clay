...
Configuration keybindings validate against a closed runtime-bindable command set. Phase 18.20 language-intelligence commands (`clay.language.hover`, `goToDefinition`, `codeActions`, `signatureHelp`) and `completion.trigger` route with `UiReactivePriority`; `Ctrl+Space` parses `Space` as the literal space key. Runtime bindings currently support one key stroke only, so manual configurations use chords such as `Alt+H`, not unsupported multi-stroke or function-key forms.
