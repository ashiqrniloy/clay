...
Configuration keybindings validate against a closed runtime-bindable command set. Phase 18.20 language-intelligence commands (`language.hover`, `goToDefinition`, `codeActions`, `signatureHelp`) and `completion.trigger` route with `UiReactivePriority`; `Ctrl+Space` parses `Space` as the literal space key. Runtime bindings accept single strokes and, since Phase 24.5, space-separated multi-stroke sequences (`"Ctrl+X Ctrl+P"`, `"g g"`); function keys remain unsupported. A multi-stroke prefix is held in the `EditorSurface` pending-chord buffer without inserting text, completes on an exact match, times out after `KEY_CHORD_PENDING_TIMEOUT_MS`, and cancels on mismatch with the key re-evaluated fresh — see [Sequence Keybindings](../modules/sequence-keybindings.md).

`EditorWidget::on_text_event` uses manifest routing for character input, Enter, and Tab. Character key events are converted to `KeyStroke` values even when Control/Alt/Super modifiers are present so configured shortcuts such as the native file-dialog command or manual completion command can reach the inert manifest router. `ClientBehaviorState::route_key` keeps modifiers exact but compares character keys case-insensitively for binding lookup; this preserves `bindKey("Ctrl+Shift+O", ...)` on Linux/GNOME where the native event text is `"O"` while the parsed manifest chord stores `"o"`. Unbound modified character keys do not insert text because `ClientBehaviorState::route_unbound_key` only falls back to text insertion when Control/Alt/Super are absent; shifted letters and symbols still insert the already-resolved character text. Autocomplete trigger declarations are classified by `ClientBehaviorState` as inert UI-reactive triggers; this classification does not run provider logic or mutate the document beyond the normal local text insertion. Local mutations still enqueue edit transactions through `ClientEditQueue::try_send`, and completion requests enqueue through the same bounded outbound channel without awaiting IPC, server work, JavaScript, file IO, AI, or full-document serialization.
...
- `src/client/behavior.rs`
  ...
  - `shifted_character_key_binding_matches_lowercase_manifest_rule`
  - `configuration_shifted_folder_binding_routes_on_linux_key_event`
