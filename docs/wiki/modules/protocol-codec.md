...
- `DEFAULT_MAX_FRAME_SIZE` is 16 MiB to prevent accidental unbounded allocation from malformed IPC frames (read-side allocation is actual frame size, never the cap); the read-side declaration gate runs before payload allocation/copy. The daemon stdio lane carries its own larger 64 MiB cap (`AGENT_DAEMON_MAX_LINE_BYTES` / `clay-agent/src/rpc.ts`) because Prism tool-result mirrors legitimately exceed 1 MiB.
...
- `tests/editor_intelligence_protocol.rs`: Phase 28 codec compatibility for the protocol version 31 handshake pin, folding, Link/InlayHint `DecorationSet`/`DecorationBatch`, inert `DecorationTarget` click/hover data, completion recency, hover request/results, and bounded malformed-frame rejection.
