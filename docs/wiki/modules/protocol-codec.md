...
The protocol module defines the first shared client/server IPC message contract for Phase 4. It uses owned Rust message types for business logic and keeps `rkyv` serialization, validation, and socket framing behind `Codec`.

## Responsibilities

- Represent initial Phase 4 messages: `Hello`, `Welcome`, `InitialDocument`, inert behavior manifests, document access, edit deltas/intents, acknowledgements, transactions, and errors.
...
`src/protocol/mod.rs` contains owned message enums and IDs. Ordinary text edits are represented as deltas (`Insert`, `Delete`, `Replace`) with byte ranges and inserted text rather than full-document payloads. `BehaviorManifest::minimal_text_editing` declares predictable built-in text editing capabilities only; it is data, not script code.
...
- `DEFAULT_MAX_FRAME_SIZE` is 1 MiB for Phase 4 to prevent accidental unbounded allocation from malformed IPC frames.
...
- `src/protocol/codec.rs`: round-trip tests for hello, initial documents with Unicode, behavior manifests, and edit deltas.
- `src/protocol/codec.rs`: rejection tests for oversized frames and invalid archived bytes.
