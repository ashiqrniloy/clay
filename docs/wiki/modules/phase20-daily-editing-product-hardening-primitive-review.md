...
| Multi-document | Server `WorkspaceState` open registry (`HashMap<DocumentId, OpenDocument>`), `open_document_snapshots`, leases, per-document metadata (`dirty`, mode, path) | Server can hold multiple open documents concurrently; opening another file issues `DocumentOpened` | Client `EditorWidget` replace-on-open (`opening_second_file_browser_file_replaces_editor_snapshot`); no client session map, tab list, or per-document caret/viewport/pending/history retention |
| Selected-file save/conflict | `WorkspaceState::{save_document,reload_document}`; dirty/stale metadata; `clay.documents.serverSaveDocument`; typed `DirtyDocument` / stale-save conflicts | Server dirty tracking, save/reload, force-reload gate, stale-metadata conflict errors that keep dirty state; Ctrl+S bindable through manifests | No user-visible dirty indicator in status chrome; no conflict/recovery transient menu; selected-file save UX incomplete before becoming user-facing; save-as/watchers/autosave remain later |
| Pending-edit/error recovery | `ClientEditQueue`, stale-edit rejection + resync, Phase 19 behavior-grace, `SduiStatusObservation`, runtime diagnostics in status text | Stale/lease rejection can resync; runtime diagnostics append to status; same-document resync preserves caret | No dedicated pending-edit HUD/prompt, reconnect/resync confirmation UX, or richer recovery menus beyond status/diagnostic text |
| Pixel / GPU snapshots | Phase 15 `SduiObservableSnapshot` / `SduiStatusObservation`; docs defer GPU snapshots | Structural headless SDUI/status regression is the hard CI layer | Masonry 0.4 exposes `masonry_testing::TestHarness` / `assert_render_snapshot`, but Clay does not depend on `masonry_testing` yet; revisit whether CI-friendly deterministic offscreen rendering is sufficient or re-defer with evidence |
...
4. **Multi-document client session:** retain per-document shadow/caret/viewport/pending/history/mode/status chrome while server remains open-registry authority; stop destroying prior session state on `DocumentOpened` replace; bound retained sessions at 64.
5. **Dirty/save/conflict UX:** surface server dirty/conflict metadata in status chrome and reuse `TransientMenuSession` for conflict/recovery prompts before selected-file save is user-facing.
...
8. **Pixel snapshot revisit:** evaluate `masonry_testing::TestHarness` / `assert_render_snapshot` against CI constraints; add pixel coverage only if deterministic offscreen rendering is proven, otherwise re-defer with evidence and keep structural observability as the hard layer.
