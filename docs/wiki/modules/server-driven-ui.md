...
`EditorWidget` paints the editor first, then overlays the current server-driven panel/list/button region and the existing status bar. Primary pointer presses that hit an SDUI action region enqueue a bounded `ClientMessage::SduiAction` through the existing client queue sender. The pointer handler does not run client-side script and does not wait for IPC capacity or server acknowledgement; ordinary text editing continues through existing editor commands and edit deltas.
...
- `src/masonry_sdui.rs`: `sdui_snapshot_replaces_native_tree_state`, `sdui_update_preserves_editor_document_state`, `editor_region_is_bounded_when_document_bound_editor_view_is_present`, `unknown_editor_view_document_uses_safe_full_editor_region`, and `stale_sdui_update_is_ignored` validate native reconciliation and editor-region composition.
