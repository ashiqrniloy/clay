...
`SduiNativeState` also implements Masonry's `Widget` accessibility hooks for the SDUI tree. Its widget root reports `Role::Group` and labels itself "Server-driven UI panels". During Masonry accessibility passes, it emits AccessKit nodes for SDUI and installed package components with the same profile-derived row rectangles that paint and hit testing use: panels use `Role::Pane`, labels use `Role::Label`, buttons use `Role::Button`, status items use `Role::Status`, lists use `Role::List`/`Role::ListItem`, editor views use `Role::MultilineTextInput`, and active transient menus use `Role::Menu`/`Role::MenuItem` with item accessibility labels. `EditorWidget` adds those nodes plus its status-bar rectangle to the mounted editor accessibility root. The helper `accessibility_nodes()` retains stable role/label traversal for headless unit tests; `ui_size_change_scales_row_hit_and_accessibility_bounds_together` locks shared bounds. Traversal is demand-driven by accessibility updates, not paint/input, and exposes no document text, absolute filesystem paths, or secrets.
