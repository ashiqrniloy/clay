...
- `src/server/facades.rs` unit test: verifies 21 unique rows and exactly 13 public-third-party rows. Representative runtime-backed sources include `runtime/js/syntax.js`, `runtime/js/completion.js`, and `runtime/js/packages.js`; each has an adjacent `.d.ts` declaration.
- `src/server/js_runtime.rs` unit tests: verify curated imports execute directly from included files, route valid package/mode/command/load/parse/decoration/UI fixtures through Rust validators, reject invalid authority, and preserve planned-unavailable exports.
- `tests/clay_js_facade_layout.rs`: verifies all 21 executable files and declarations expose matching functions, every JavaScript file is included exactly once by the runtime table, no raw-string facade remains in `js_runtime.rs`, and exports reject op-shaped/redundant names.
