...
- Define stable domain module files for `clay:editor`, `clay:keybindings`, `clay:configuration`, `clay:documents`, `clay:workspace`, `clay:behavior`, `clay:application`, `clay:sdui`, and `clay:ui`, plus runtime module sources for primitive gates under `clay:packages`, `clay:modes`, `clay:commands`, `clay:decorations`, and `clay:parse`.
...
- `src/server/js_runtime.rs` unit tests: verify that `clay:packages`, `clay:modes`, `clay:commands`, `clay:decorations`, `clay:parse`, and `clay:ui` import in the controlled runtime, route valid package/mode/command/load/parse/decoration/UI contribution fixtures through Rust validators, return actionable Clay error codes for invalid permissions, reject invalid package UI declarations, keep unpromoted folding/selection/configuration APIs explicitly planned/unavailable, and keep reviewed package/mode/UI override configuration setters as planned-unavailable facade exports.
- `tests/clay_js_facade_layout.rs`: verifies expected domain files and planned/runtime-backed exports exist, including Phase 9 `clay:documents` file lifecycle APIs, `clay:workspace` root metadata, Phase 12/13 `clay:sdui` helper/publication exports, and Phase 18.3 `clay:ui` contribution exports, rejects raw op-shaped exports, rejects redundant names, and checks facade files do not call raw `Deno.core.ops` dot-call syntax.
