...
- Define stable domain module files for `clay:editor`, `clay:keybindings`, `clay:configuration`, `clay:documents`, `clay:workspace`, `clay:behavior`, `clay:application`, and `clay:sdui`, plus runtime module sources for primitive gates under `clay:packages`, `clay:modes`, `clay:commands`, `clay:decorations`, and `clay:parse`.
...
Each facade file exports TypeScript option/result types and functions for its domain. Runtime-generated ESM equivalents are resolved by `ClayModuleLoader` for curated specifiers such as `clay:configuration`, `clay:sdui`, `clay:documents`, `clay:workspace`, `clay:keybindings`, `clay:behavior`, `clay:packages`, `clay:modes`, `clay:commands`, `clay:decorations`, and `clay:parse`. The `runtime/js/packages.ts`, `runtime/js/modes.ts`, `runtime/js/commands.ts`, `runtime/js/decorations.ts`, and `runtime/js/parse.ts` source files mirror the controlled-runtime primitive facade strings so API inventory/doc tests can verify stable exports even though the embedded runtime currently loads string-backed modules. The generated facades call Clay-owned ops internally; configuration/package code imports documented functions and does not call raw `Deno.core.ops.op_*` names. `serverPublishDecorations` and `serverRegisterParseHandler` now call explicit ops that validate package permissions/provenance and bounded provider metadata. Deferred APIs in editor and application modules, older document lease/snapshot helpers, package loading, package/mode configuration setters, `serverSelectDocumentManifest`, and folding provider hooks throw clear planned-unavailable errors through a shared op. In `runtime/js/documents.ts`, Phase 9 document metadata mirrors the protocol's document id, version, access/lease, dirty flag, workspace root id, and workspace-relative path. `serverOpenDocument` and `serverReloadDocument` are explicit snapshot-returning server-first calls; `serverSaveDocument`, `serverGetDocumentStatus`, and `serverListDocuments` return metadata only. In `runtime/js/workspace.ts`, `serverListWorkspaceRoots` exposes configured server root metadata without path expansion or file access. In `runtime/js/keybindings.ts`, `bindKey` and `unbindKey` accept `global`/`editor` scopes while `listKeyBindings` accepts an additional `all` scope filter to match the documented configuration query default. In `runtime/js/sdui.ts`, helper names mirror the Rust SDUI node kinds (`Panel`, `Label`, `Button`, `List`, `EditorView`, `Flex`, and `Stack`) as lower-camel-case `define*` exports backed by `op_clay_sdui_define_node`; `publishTree` calls `op_clay_sdui_publish_tree`, which converts the JSON object graph into a typed validated server `SduiTree`.

`runtime/js/mod.ts` re-exports the domain files as namespaces for source-tree organization and tests. Runtime import-map work can later map the individual files to Clay-owned module specifiers such as `clay:editor`, `clay:keybindings`, `clay:application`, and `clay:sdui`.
...
- `src/server/js_runtime.rs` unit tests: verify that `clay:packages`, `clay:modes`, `clay:commands`, `clay:decorations`, and `clay:parse` import in the controlled runtime, route valid package/mode/command/load/parse/decoration fixtures through Rust validators, return actionable Clay error codes for invalid permissions, keep unpromoted folding/selection/configuration APIs explicitly planned/unavailable, and keep reviewed package/mode configuration setters as planned-unavailable facade exports.
- `tests/clay_js_facade_layout.rs`: verifies expected domain files and planned exports exist, including Phase 9 `clay:documents` file lifecycle APIs, `clay:workspace` root metadata, and Phase 12/13 `clay:sdui` helper/publication exports, rejects raw op-shaped exports, rejects redundant names, and checks facade files do not call raw `Deno.core.ops` dot-call syntax.
...
- `tests/clay_js_doc_registry.rs`: verifies the generated registry contains the package/mode/command primitive gate API docs plus promoted `serverLoadPackage`, while still excluding planned-only Phase 18/provider surfaces.
...
- Relevant commands: `cargo test clay_js_facade --test clay_js_facade_layout`, `cargo test --test package_loading_docs`, `cargo test --test clay_js_api_inventory`, `cargo test --test clay_js_doc_registry`, `cargo test --test primitives_docs`, and `cargo test --test rust_visibility_api_mapping`.
