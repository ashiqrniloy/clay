...
| Package records | `assemble_package_record` in `src/packages/record.rs` | Produces typed package records with contribution provenance for commands, key routing, configuration, SDUI, decorations, package UI, input, state scopes, layout overrides, package options, docs, performance, and API dependencies. | Needs source identity and authorization identity in the record so diagnostics and revocation can find all package-owned contributions. |
