...
- `src/client/behavior.rs`: validates atomic client replacement, previous-manifest retention on invalid replacement, client-first key routing, Tab routing, autocomplete trigger classification, manual `completion.trigger` routing, and server-first intent routing.
