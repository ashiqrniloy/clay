...
- The editor accessibility root publishes the same profile-derived status rectangle and includes the hosted `region`/`panel_host`/`overlay_host` child ids in its accesskit children (gated on presence); SDUI/panel/overlay accessibility flows through the hosted Masonry widget subtree. Accessibility updates are requested only after accepted UI/profile/tree changes.
