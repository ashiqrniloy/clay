...
- Syntax/semantic decorations resolve through `StyleRegistry::style_for` and become ranged Parley foreground brushes, so theme colors change glyph color rather than painting highlight rectangles behind text. The cached layout owns the brush table; selection and diagnostic squiggles remain separate native paint layers. The Clay default provides distinct keyword/string/comment/operator/prose colors, while active themes can override every `TokenType` independently.
