...
- Keep non-`@clay/*` resolver guards centralized in package load/enable/resolver code, not mode-specific branches.
...
- Future hardening tests should cover production sandbox routing, third-party rejection, and sanitized diagnostics once the harness graduates beyond test-only use.
