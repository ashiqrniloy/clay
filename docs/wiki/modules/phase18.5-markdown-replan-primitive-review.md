...
- One-line end-user package loading      -> loadPackage("@clay/markdown") (GAP)
```

Every Markdown need except one-line end-user package loading maps onto an implemented generic primitive. No Markdown-specific Rust editor/parser/render/shell branch is required.

## Generic Phase 18.5 Primitive Gaps

### `loadPackage("@clay/markdown")` — generic one-line package specifier resolver

The only generic gap blocking the Markdown replan is the absence of a generic end-user package loader that resolves an installed package specifier, reads its declared `loadEntry` (e.g., `./dist/load.js`), enables the package through the existing `PackageService` validation path, and executes the package-owned load entry against the validated `PackageRecord`.

Today only `serverLoadPackage(packageJson)` exists. It requires the caller to construct the inline package manifest object — exactly what the fixture `init.js` files at `tests/fixtures/configuration/markdown-mode/init.js` and `tests/fixtures/configuration/windows-markdown-open/init.js` do today, and exactly what Plan 023 intends to remove from ordinary end-user config. The Markdown package's own `loadMarkdownPackage(clay, options)` entry already encapsulates mode pattern registration, major-mode activation, command registration, and parse handler registration; what is missing is a generic resolver that invokes it from a one-line specifier.

Candidate public API target: `clay.packages.loadPackage` / `loadPackage("@clay/markdown")`, exported from `runtime/js/packages.ts` (or a new `clay:packages` facade entry) and backed by an op such as `op_clay_packages_load_package_by_specifier` in `src/server/ops/packages.rs` plus a specifier resolver in `src/packages/service.rs`.

The resolver must remain deny-by-default for arbitrary external specifiers. Acceptable scope for a safe Phase 18.5 implementation is a constrained first-party resolver for `@clay/*` specifiers that resolves against the package distribution directory, validates the manifest through the existing `PackageService`, and executes the declared `loadEntry` with the `clay` facade object. External package resolution, package-manager execution, registry fetching, and arbitrary specifier expansion remain prohibited unless a future approved decision log adds them.

If authority, validation, or scope constraints prevent a safe Phase 18.5 implementation, the gap is documented in a decision log with a temporary package-owned fallback entry such as:
...
The fallback must still consume implemented generic primitives (`serverLoadPackage`, `serverRegisterModePattern`, `serverActivateMajorMode`, `serverRegisterCommand`, `serverRegisterParseHandler`) internally rather than fixture-only copied manifests or inline package objects in user `init.js`. Either way, the end-user `init.js` must be concise and the explicit `bindKey("Ctrl+O", "clay.documents.clientOpenFileDialog", { scope: "editor" })` separation is preserved.
...
- Package loading is deny-by-default for arbitrary external specifiers, package-manager execution, registry fetching, and arbitrary specifier expansion. A constrained first-party `@clay/*` resolver (if implemented) must reuse the existing `PackageService` validation path and execute the declared `loadEntry` against the validated `PackageRecord`.
...
- None of these primitives grant filesystem (outside already-open document content and the config root), network, shell, extension loading, AI mutation, workspace mutation, package installation/enable/disable, package-manager execution, WASM, raw `Deno.core.ops`, native widget, direct Masonry widget, renderer callback, raw CSS, or client-side JavaScript authority by default.
...
- Do not implement the one-line loader as a Markdown-specific resolver. The loader must be a generic `loadPackage(specifier)` that any first-party package can consume; Markdown only publishes its `loadEntry`.
...
3. Implement a constrained first-party `loadPackage("@clay/*")` resolver if it can be done safely within the phase; otherwise document the gap in a decision log and ship a package-owned fallback entry that consumes generic primitives internally.
...
- Generic gap review: the only generic gap blocking the Markdown replan is `loadPackage("@clay/markdown")` as a one-line specifier resolver; it must remain a generic primitive and must not become a Markdown-specific loader.
...
- `tests/primitives_docs.rs::phase18_5_markdown_replan_primitive_review_records_existing_inventory`
- `tests/primitives_docs.rs::phase18_5_markdown_replan_primitive_review_maps_markdown_to_generic_primitives`
- `tests/primitives_docs.rs::phase18_5_markdown_replan_primitive_review_identifies_load_package_gap`
- Command: `CARGO_TARGET_DIR=target/pi-verify cargo test --test primitives_docs --quiet`
