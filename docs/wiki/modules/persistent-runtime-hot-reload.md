...
- Route explicit user reloads through the built-in `runtime.reloadConfiguration` command and the same `CommandExecutor` validation used by keybindings, SDUI actions, transient menus, and Control Center.
...
- Acquire the server-owned behavior lock (`ScopedLockManager` in `src/server/locks.rs`) only for candidate compare-and-swap commit and release it before document refresh.
...
Current Phase 19 boundary (Plan 054 complete): candidate preparation/commit through two-phase validate-then-mutate, generic behavior-scoped command serialization via `ScopedLockManager`, post-commit generation-owned contribution replacement/cleanup (`cancel_older_runtime_generations`, `shutdown_trusted_generation_resources`), live `RuntimeStateSnapshot` fan-out (capacity-16 broadcast, latest-snapshot lag recovery), atomic client install via `ClientRuntimeStateCandidate` (validate-then-mutate, fail-close on invalid, ack after install), bounded previous-generation stale-edit grace (`BehaviorGraceState`, 2s/256-transaction ceilings, `InvalidBehaviorVersion` resync), preserved one-line `loadPackage` reload lifecycle docs, documented explicit built-in `runtime.reloadConfiguration` command with compiled-budget table, rejected hidden keys, verified Clay JS API/command metadata boundaries (pub(crate) lifecycle helpers, no JS facade), and end-to-end verification tests (duplex barrier, multi-client atomic install, authority denial, LSP cleanup, budget locks). Full Linux gates passed.
...
3. Configuration reruns `~/.clay/init.js`; package authors normally call `await loadPackage("@clay/markdown")` there. `runtime/js/packages.js` keeps `globalThis.__clayLoadedPackages` as a per-generation idempotence cache.
...
- Language-server generation teardown: `LanguageServerProcessService::shutdown_all` via `ClayJsRuntimeService::shutdown_trusted_generation_resources` (trusted grants/process authority end at commit; the shared third-party domain and its language-server sessions survive a trusted reload — Plan 061 task 12).
- Open-document refresh primitive: `WorkspaceState::open_document_refreshes` plus `refresh_open_documents_after_reload`. Refresh uses metadata plus current document handles; classification reads a bounded leading-content slice, native parse uses `DocumentState::parse_window_snapshot`, and document analysis copies text only below `DOCUMENT_ANALYSIS_MAX_DOCUMENT_BYTES`.
- Scoped-lock primitive: behavior-only `ScopedLockManager` with RAII `ScopedLockGuard` in `src/server/locks.rs`. Plan 131 deleted the unwired range/document/workspace targets and the shared `ranges_overlap` helper; region-lock overlap checks are internal to `DocumentState`.
...
- Runtime snapshot fan-out primitive: `ActiveRuntimeStateFanout` in `src/server/mod.rs` plus `src/protocol/runtime.rs` (`RuntimeStateSnapshot`, `RuntimeGenerationInstalled`). Connections subscribe, stamp `client_id` on send, and recover from `latest_runtime_snapshot_for` after lag. Since Plan 132 the notification channel is a `fanout::Fanout<RuntimeGenerationId>` (capacity `RUNTIME_STATE_BROADCAST_CAPACITY`) — only the generation id travels on the lane, while the snapshot store and the per-client acknowledgement map stay outside it, because the snapshot is narrowed per client (`for_client`) rather than replayed verbatim. See [Server State Fanout Lanes](server-state-fanout.md).
...
- Reload preserves module loading through recorded package allowlist entries and resolver-validated package `loadEntry` imports; `PackageLoadEntryAllowlist::revoke_package` is the withdrawal hook, but no production disable/revoke path calls it yet (plan 131 task 4 finding), so recorded entries survive a disable for the current generation.
...
- `cargo test --lib locks::tests`: behavior-lock `BehaviorLocked` conflict and RAII release semantics.
