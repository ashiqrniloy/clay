...
- Validate stale versions, ranges, optional parse-produced decoration metadata, known decoration style tokens, `DECORATION_PAYLOAD_BUDGET_BYTES`, and `INCREMENTAL_PARSE_UPDATE_BUDGET_BYTES` before publishing updates to downstream consumers.
...
- Token-stream adapters receive package-neutral parse metadata (`document_id`, versions, package prefix, mode ID, viewport, and invalidated byte ranges). If a future adapter needs line-start tables or range snapshots, those must be added as generic parse-input primitives, not mode-specific Rust parser branches.
