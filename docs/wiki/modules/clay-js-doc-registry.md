...
- Include Phase 8 configuration entry point APIs (`configuration.loadConfigurationModule` and `configuration.getConfigurationState`) so app/help/agent lookup can discover `~/.clay/init.js` and local modular configuration semantics without executing user code.
...
- Enforce the Plan 060 public-surface audit: the current inventory has 148 classified rows mapping to 130 generated public entries, while trust-domain identity, package context, output routing, close lifecycle, queue/file/scheduler budgets, and file identity remain private or `pub(crate)` and absent from every executable/declaration facade.
- Plan 090 API verification found no new public programmatic capability: extracted server/runtime/editor/shell/driver helpers remain private or `pub(crate)`, existing facade IDs/exports remain stable, and moved Rust owners are recorded in authoritative Markdown/inventory metadata, including `src/driver/restore.rs::Driver::apply_tab_command` and `src/app_driver.rs` routing.
- Phase 28 promotes five client command-ID helpers through the existing trusted `clay:editor` facade: `toggleComment`, `toggleListMarker`, `rotateHeading`, `clientToggleFold`, and `toggleInlayHints`. They are synchronous, side-effect-free naming helpers used with `bindKey`; their routed behavior remains native/client-local and has no new Deno op or package authority. The configuration review keeps `Ctrl+/` on `editor.toggleComment`, keeps wrap overrides on `editor.clientSetEditorLayout`, keeps chrome/inlay defaults in `editorRules.chrome`, and adds no new package option keys.

The registry code does not execute JavaScript, load `~/.clay/init.js`, call Deno ops, grant permissions, or run on editor paint/input hot paths.
...
- `tests/clay_js_doc_registry.rs::phase28_folding_api_uses_reserved_core_domain`: verifies `folding` remains a reserved core domain and the folding publication API is discoverable with its public module/export and `render-folding` permission.
- `tests/clay_js_doc_registry.rs::phase28_editor_command_apis_are_documented_and_facaded`: verifies the five Phase 28 command-ID helpers have matching JS/TypeScript exports, docs/index links, inventory/generated-registry metadata, default key bindings, and denied-authority security notes.
- `tests/clay_js_doc_registry.rs::phase28_configuration_apis_have_documented_bindings_and_closed_options`: verifies the Phase 28 configuration guide and `bindKey` docs list every bindable command, preserves the closed option stance, and keeps `ui-session` in the `setPackageOption` TypeScript/registry metadata.
...
- `tests/clay_js_doc_registry.rs::configuration_entrypoint_is_documented_and_indexed`: verifies `~/.clay/init.js` is documented and configuration facade entries are generated.
...
- `tests/rust_visibility_api_mapping.rs::third_party_facade_allowlist_exactly_matches_plan_public_inventory`: verifies the 14-facade third-party allowlist remains exact, including `clay:folding`.
...
- `tests/rust_visibility_api_mapping.rs::phase22_8_per_tab_state_has_no_new_public_programmatic_surface`: keeps per-tab state/routing internal and rejects arbitrary tab handles in runtime JS.
...
- Plan 090 verification commands: `cargo test --test security rust_visibility_api_mapping::`, `cargo test --test protocol clay_js_ -- --test-threads=1`, and `cargo run --bin update-doc-registry` (the only mutating registry command).
