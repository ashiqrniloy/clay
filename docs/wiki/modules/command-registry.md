...
- `clay.workspace.toggleFileBrowser` returns `WorkspaceActionResult::Toggled`; the bound connection applies the per-tab visibility flag and publishes either the bounded tree or an editor-only SDUI snapshot.
