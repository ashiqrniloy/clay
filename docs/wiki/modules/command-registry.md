...
- Phase 18.12 adds four built-in file-browser commands (`clay.workspace.openFile`, `clay.workspace.revealInTree`, `clay.workspace.openFuzzyFile`, `clay.workspace.toggleFileBrowser`) and a server-side workspace execution path (`CommandExecutor::execute_workspace`). Open commands resolve through `WorkspaceState::open_existing_file` for in-root paths and `WorkspaceState::open_selected_file` for out-of-root absolute paths, enforcing the selected-file single-file grant flow. Reveal and toggle are accepted as inert UI state transitions. Save/save-as/rename/delete actions remain unregistered and deferred per roadmap.
