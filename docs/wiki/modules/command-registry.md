...
- Phase 18.12 adds four built-in file-browser commands (`workspace.openFile`, `workspace.revealInTree`, `workspace.openFuzzyFile`, `workspace.toggleFileBrowser`) and a server-side workspace execution path (`CommandExecutor::execute_workspace`). Open commands resolve through `WorkspaceState::open_existing_file` for in-root paths and `WorkspaceState::open_selected_file` for out-of-root absolute paths, enforcing the selected-file single-file grant flow. Reveal is accepted as an inert UI state transition; toggle is validated here and applied by the bound connection to its per-tab workspace-pane visibility state. Global chords (`Ctrl+X Ctrl+O` → `controlCenter.open`, `Ctrl+X Ctrl+P` →
`shell.toggleAgentLane`) resolve in the shell key handler outside editor focus
— the editor keymap owns them inside `.cm-editor` only — and dispatch through
the active tab session; the lane toggle then flips client-local persisted layout
state rather than advancing the runtime generation. The shell filter normalizes the serialized enum names before comparing — serde emits unit-variant names as written (`"Global"`/`"ServerFirst"`), and package strings may use kebab (plan 108 exit-gate fix: without the shell matcher the Command Centre was unreachable with no document open). The composer-owned palette closes via Escape (and clears its local reopening guard on draft change); row activation clears the draft, while `menuCancel` clears the menu locally before notifying the server so dismissal never waits on the round trip. Plan 125 lists the picker-stage ids (`agent.clientOpenAgentPicker`, `agent.clientOpenProviderPicker`, `agent.clientOpenModelPicker`, `agent.clientOpenProviderSetup`, `agent.clientOpenSessionPicker`, `agent.clientOpenSessionSearchPicker`, with the labels `Choose Agent`/`Choose Provider`/`Choose Model`/`Configure Provider`/`Resume Session`/`Search Sessions`) as palette rows: they are built-in server-first ids that the palette maps to stages, deliberately **not** `bindKey` targets and not package-executable, so a package cannot open or drive a picker stage. Save/save-as/rename/delete actions remain unregistered and deferred per roadmap.
...
Connection command intents and validated SDUI/menu actions call `IpcServer::execute_reload_command`. That method reruns shared `CommandExecutor` validation, rejects a concurrent attempt with `CommandExecutionRule::ReloadInProgress`, evaluates a fresh candidate outside scoped locks, and acquires the behavior lock (`ScopedLockManager::try_acquire`) only for compare-and-swap commit. RAII drops both attempt and behavior guards on every return/unwind path. Package-side `serverExecuteCommand` rejects this ID with `UnauthorizedTarget`; package JavaScript can declare UI/key routing but cannot directly invoke reload authority. No reload-specific client message or dispatcher exists.

`src/server/locks.rs` supplies one immediate, non-waiting behavior-scope lock: `ScopedLockManager::try_acquire` either returns a RAII `ScopedLockGuard` or the typed `BehaviorLocked` conflict, so callers cancel instead of queueing. Plan 131 deleted the unwired range/document/workspace targets and the shared overlap helper; document-region overlap checks stay internal to `DocumentState`.
