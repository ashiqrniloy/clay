...
9f. The Phase 18.9 [Generic Text/Code Modes Primitive Review](../archive/phase18.9-generic-text-code-modes-primitive-review.md) inventories existing `ModeRegistry` classification/activation, behavior-manifest `EditorBehaviorRules` (`Enter`/`Tab`/`pairs`/`comments`/`ContinueLineMarkers`/`PreserveFenceBodyIndent`/`AutocompleteTrigger`), keybinding/command-execution, SDUI/status, decoration, and document-open primitives. It records that most generic key behavior already exists as `TextTransform` manifest data and that the genuine gaps are narrower: always-on built-in `core.text`/`core.code` fallback modes registered through `MajorModeActivation`/`DocumentClassification` (no new primitive), classification shebang and bounded leading-content probes plus a documented precedence ladder, an electric-character manifest kind extending `EnterRule`/`PairRule`, and mode discovery/listing commands as read-only `CommandDeclaration` consumers routed through `CommandExecution`. It rejects language-specific Rust classification/transform branches, package-shaped built-in modes requiring `loadPackage`, parallel fallback registries, and any new filesystem/network/shell/AI/WASM/raw-op/client-JS/package-manager authority.
9g. The Phase 18.10 [Tree-sitter Grammar Primitive Review](../archive/phase18.10-tree-sitter-grammar-primitive-review.md) inventories existing document classification, major-mode activation, package loading, parse coordinator, decoration transport, style-token validation, package manifests, package permissions, docs registry, and wiki coverage before language package work. It records the generic `SyntaxGrammarContribution` gap, plus grammar registry, query/capture validation, capture style mapping, and syntax provider selection as separate from active major mode. It rejects Rust/TypeScript/JavaScript-specific Rust parser branches, full-mode language packages in this phase, arbitrary native/third-party grammar artifact loading, and any new filesystem/network/shell/AI/WASM/raw-op/client-JS/package-manager authority.
9h. The Phase 18.11 [Completion Provider Framework Primitive Review](../archive/phase18.11-completion-provider-primitive-review.md) inventories existing behavior-manifest autocomplete triggers, client behavior routing and local-edit path, command registry/execution and the built-in `completion.trigger` command, `TransientMenuSession`/overlay projection, mode registry fallback modes, parse-coordinator cancellation/generation model, syntax grammar registry/provenance, decoration/payload budgets, package permissions and disable/revocation withdrawal, and docs/wiki coverage before provider work. It records the generic `CompletionTriggerAndResult` gaps: typed `CompletionRequest`/`CompletionResultSet`/`CompletionItem` protocol shapes, a `CompletionProviderRegistry`, a cancellable server-side `UiReactivePriority` `CompletionCoordinator`, behavior-manifest trigger routing and manual trigger, a `TransientMenuSession` completion display/accept adapter, a built-in buffer-word provider, and a `completion.serverRegisterCompletionProvider` registration API. It rejects language-specific Rust completion branches, completion-only menu widgets, provider JavaScript in client hot paths, synchronous IPC before local paint, executable snippet/command side effects on accept, and any new filesystem/network/shell/AI/workspace/WASM/raw-op/native-widget/client-JS/package-manager/package-enable authority.
9i. The Phase 18.12 [Workspace Discovery and File Browser Foundation Primitive Review](../archive/phase18.12-workspace-discovery-primitive-review.md) inventories existing `WorkspaceState` multi-root roots/grants, shell `FixedSlotId::Left`/`Bottom` slots, `CommandExecution`, `TransientMenuSession`, SDUI/component catalog, selected-file grants, and fallback-mode activation. It records that the left fixed-panel file tree and bottom transient fuzzy-open are compositions of existing primitives, not new primitive categories. The generic gaps are a server-owned workspace-root discovery helper (cwd/CLI, opened-file ancestry, explicit user grants, bounded marker files) and a bounded server file tree/list service (ignore rules, depth/count limits, cancellation, refresh, diagnostics). It rejects file-browser-specific Rust widgets/branches, client-side discovery, package root/marker/ignore authority, raw path open ops, and any new filesystem/network/shell/AI/WASM/raw-op/native-widget/client-JS/package-manager/package-enable authority.
9i2. The [End-to-End File Browser Workflow Primitive Review](../archive/end-to-end-file-browser-workflow-primitive-review.md) maps the six-step product workflow onto existing workspace, SDUI file-browser, command execution, selected-file, language package, and editor selection primitives. It records the remaining generic gaps: selected-folder client UI grants, file-browser directory navigation state, generic open-document follow-up activation for workspace/file-browser opens, and client-owned copy-selection clipboard write. It rejects package-owned file browsers, raw path opens, shell-backed dialogs, language-specific Rust open branches, and server/package clipboard authority.
9i3. The [Manual File Browser Workflow Bugfix Primitive Review](../archive/manual-file-browser-workflow-bugfix-primitive-review.md) inventories existing `KeyRoutingOverride`/behavior manifests, `ClientUiCommandRoute`, `FileBrowserState`, `StaticSduiState`, `SduiNativeState`, `PaneSlotLayout`, `EditorSurface` scroll/paint state, and open-document follow-up primitives before Plan 044 fixes. It maps the real `cargo run` Linux/GNOME bug reports to generic primitive owners: shifted key matching, SDUI row/action identity, workspace-browser validation-state separation, document-open replacement, editor-region reservation, editor paint chrome, file-browser local scroll, and editor scrollbar chrome. It rejects Markdown/Rust-specific Rust branches, validation relaxation, server-side scrolling, hidden config keys, package-owned native widgets, raw CSS/native handles/client-side JavaScript, and raw `Deno.core.ops` authority.
9j. The Phase 18.13 [Git Discovery Service Primitive Review](../archive/phase18.13-git-discovery-primitive-review.md) inventories existing `WorkspaceState` roots, Phase 18.12 workspace/file-browser authority, `CommandExecution`, `TransientMenuSession`, Control Center, package loading, package UI/status components, configuration APIs, and documentation registry coverage before first-party `@clay/git` work. It records that `@clay/git` can be an explicitly loaded package consuming typed cached Git status APIs and inert UI/action primitives. The generic gaps are a server-owned read-only `GitDiscoveryService`, a per-workspace `GitStatusCache`, and documented `clay:git` facades. It rejects arbitrary shell, network/remotes, mutating Git commands, package process/filesystem authority, client-side Git execution, raw ops, and Git-specific native widgets.
9k. The Phase 18.16 [Tiered Tree-sitter Syntax Engine Primitive Review](../archive/phase18.16-tiered-tree-sitter-engine-primitive-review.md) inventories the existing `SyntaxGrammarRegistry`, `ParseCoordinator`, decoration transport, Phase 18.15 `TokenType`/`Modifiers` and `StyleRegistry`, package loading, `clay:syntax`, and JS parse handler bridge, then records the completed generic tier-engine implementation. The delivered primitives are `SyntaxEngineTier`, engine selection/provenance, native first-party descriptors, a shared web-tree-sitter host adapter contract, retained JS parser fallback, one capture-to-vocabulary mapper, and open-parse `RuntimeDiagnostic` publication. It preserves rejection of per-language Rust highlighter branches, a second parse scheduler, parser/query/package JS work in hot paths, raw renderer artifacts, and silent package override of Tier 1 native highlighting.
9l. The Phase 18.16.5 [Semantic Typography Primitive Review](../archive/phase18.16.5-typography-primitive-review.md) maps existing theme delivery, decoration transport, cached Parley layout, viewport/scroll state, SDUI/package UI, and package validation to user-owned `monospace`/`proportional`/`ui` profiles. Its generic gaps are a separate atomic typography snapshot/registry, semantic role data, normalized layout runs, revision-aware cache/geometry, and shared UI typography metrics. It preserves `StyleRegistry` color ownership, bounded visible extraction, and rejection of package-selected concrete fonts, full-document shaping, and language-specific font branches.
9m. The Phase 18.17 [Range Diagnostics Primitive Review](../archive/phase18.17-range-diagnostics-primitive-review.md) maps existing `RuntimeDiagnostic` status reporting, decoration transport/chunk caches, `ParseCoordinator`, tiered Tree-sitter handling, `StyleRegistry`, cached Parley geometry, native Vello paint, and package publication validation to inline diagnostics. It identifies generic diagnostic metadata, source replacement, a parse side channel, source-keyed client chunks, severity-owned theme styles, and one bounded package publication API. Current implementation reserves publication for explicit analyzers; Tree-sitter recovery nodes are not correctness authority. It rejects language-specific providers, metadata encoded as style tokens, a second scheduler, hot-path parser/package work, and Phase 18.17 LSP process/provider scope.
9n. The Phase 18.18 [First-Party Language Package Full Implementation Primitive Review](../archive/phase18.18-language-package-primitive-review.md) inventories the post-18.17 primitive surface — `FIRST_PARTY_NATIVE_GRAMMARS` Tier 1 native registration, the two-axis `DecorationSpan` + `StyleRegistry`, `buildCodeEditingManifest`, `serverRegisterCompletionProvider`, the parse coordinator's decoration/diagnostic side channels, and the four Phase 18.14 grammar-only packages — and records that the central generic gap is promoting first-party grammar styleMaps from the lossy free-form `style_token` compatibility fallback to direct Phase 18.15 `TokenType` + `Modifiers`, plus expanding `queries/highlights.scm` captures, shipping priority-0 keyword completion providers (snippets deferred to Phase 18.19), tuning full mode behavior on generic primitives, and moving Markdown decoration onto the vocabulary contract while preview stays package-JS. It rejects per-language Rust syntax/manifest/completion branches, vocabulary encoded as style-token strings, color data in styleMaps, Markdown preview routing through Tree-sitter, silent package defaults, and Phase 18.18 LSP process scope.
9o. The Phase 18.20 [Language Intelligence Primitive Review](../archive/phase18.20-language-intelligence-primitive-review.md) inventories existing semantic decoration, diagnostic, completion, command/menu, workspace navigation, document-version, package authorization, runtime handler-token, and internal process primitives. It records three generic gaps: engine-neutral `LanguageIntelligenceRequestAndResult` types, one feature-tagged `LanguageIntelligenceProvider` lane, and a separately approved `LanguageServerSession`. LSP line/character/URI/JSON-RPC conversion remains package-side; Clay core keeps UTF-8 byte offsets, bounded inert results, existing UI/navigation composition, no language-specific branches, and no false filesystem-sandbox claim.
9p. The Phase 18.21 [LSP Bridge Primitive Review](../archive/phase18.21-lsp-bridge-primitive-review.md) verifies that process grants/sessions, intelligence results, semantic decorations, diagnostics, completion presentation, navigation, commands, package loading, and all four base language packages are reusable as-is. It locks four generic gaps only: byte-exact session transport, a decision-gated bounded document-analysis worker lifecycle, a resolver-token adapter into the existing completion coordinator, and long-lived worker outputs routed through existing validators. LSP framing/types/methods/capabilities/positions/URIs and per-server policy remain package JavaScript; no language-specific Rust branch or parallel renderer/provider/process path is allowed.
10. Phase 18.1 finalized the canonical shell/layout reference in `docs/reference/primitives/shell-layout-strategy.md`, and the registry/backlog rows now define `WorkingAreaLayout`, `PaneSplitTree`, `PaneSlotLayout`, `PanelContribution`, `ComponentContribution`, `TransientOverlayContribution`, `PackageThemeTokenDeclaration`, `PackageUiStateScope`, `PackageLayoutOverride`, `CommandExecution`, `TransientMenuSession`, `ControlCenter`, workspace-root discovery, and bounded file tree/list service categories. The rows keep Masonry/native widgets internal, record no-hot-path policy and bounded payload expectations, and map the primitives to Phase 18.2, 18.3, 18.4, 18.8, and 18.12 implementation queues.
...
- [Phase 18.1 Shell/Layout Primitive Review](../archive/phase18.1-shell-layout-primitive-review.md)
- [Phase 18.2 Shell Runtime Primitive Review](../archive/phase18.2-shell-runtime-primitive-review.md)
...
- [Phase 18.9 Generic Text/Code Modes Primitive Review](../archive/phase18.9-generic-text-code-modes-primitive-review.md)
...
- [Manual File Browser Workflow Bugfix Primitive Review](../archive/manual-file-browser-workflow-bugfix-primitive-review.md)
...
- [Phase 18.16 Tiered Tree-sitter Syntax Engine Primitive Review](../archive/phase18.16-tiered-tree-sitter-engine-primitive-review.md)
- [Phase 18.16.5 Semantic Typography Primitive Review](../archive/phase18.16.5-typography-primitive-review.md)
- [Phase 18.17 Range Diagnostics Primitive Review](../archive/phase18.17-range-diagnostics-primitive-review.md)
- [Phase 18.18 First-Party Language Package Full Implementation Primitive Review](../archive/phase18.18-language-package-primitive-review.md)
- [Phase 18.20 Language Intelligence Primitive Review](../archive/phase18.20-language-intelligence-primitive-review.md)
...
- [Phase 18 Markdown Primitive Review](../archive/phase18-markdown-primitive-review.md)
