...
8. A changed visible byte range enqueues a deduplicated `DecorationViewportRequest` through the existing nonblocking client queue only when doing so leaves one slot for user intent. The server validates client/document/version/range metadata, prepares one UTF-8-safe parse window capped by its `ParsePolicy`, and schedules contiguous 256-byte decoration viewport tasks over that shared window. Viewport start is part of the coordinator task key, so sibling chunks coexist instead of canceling each other; each newer viewport group first cancels unfinished chunks from the older group.
...
After each accepted workspace edit, `refresh_native_syntax_after_edit` schedules the existing native syntax handler around the changed byte offset, capped by the grammar's `ParsePolicy.max_window_bytes`. On the client, `EditorDecorationState::apply_edit` immediately shifts decoration spans after the local edit and drops only spans intersecting the changed range. `EditAck` then advances the retained decoration state's version instead of clearing every chunk. Unaffected syntax colors therefore remain stable while the scheduled native/LSP replacement arrives; resync and document-open snapshots still clear all caches. This reuses `schedule_parse_window`; no full-document parse or language-specific branch enters the edit path.
...
- Decorations are viewport/chunk-bounded; spans outside the declared viewport are rejected. Opening schedules contiguous chunks across the first bounded window, then deduplicated viewport requests schedule later chunk groups without document text crossing from client to server.
...
- `src/editor/surface.rs`: verifies Markdown code uses monospace inside a proportional document, unaffected decoration spans shift and survive local edit acknowledgement without flicker, overlap resolution/attribute composition is deterministic, stale earlier spans cannot underflow after scrolling, and diagnostic or invalid UTF-8 spans cannot affect font roles; `src/editor/layout.rs` verifies typography/style/default-role cache invalidation.
- `src/masonry_editor.rs::scrolling_enqueues_new_decoration_viewport_once`, `src/client/mod.rs::decoration_viewport_request_emits_bounded_range_metadata`, and `src/server/connection.rs::native_viewport_chunks_decorate_all_first_party_languages_beyond_first_lines` cover deduplicated client requests and nonzero native parse windows.
