...
8. A changed visible byte range enqueues a deduplicated `DecorationViewportRequest` through the existing nonblocking client queue. The server validates client/document/version/range metadata and schedules the registered native handler with a UTF-8-safe window capped by its `ParsePolicy`.
