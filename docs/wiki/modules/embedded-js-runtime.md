...
Clay runs two **trust domains** per `ClayJsRuntimeService`: one **Trusted** domain for Clay core, bundled first-party packages, and user configuration evaluation, and one **ThirdParty** domain for user-authorized third-party packages. The split is structural, not profile-based: `RuntimeDomain::Trusted` is granted by a compiled bundled inventory (`BUNDLED_PACKAGES` with FNV-1a-64 fingerprints over canonical roots), and everything else is `ThirdParty` with no promotion path into the trusted domain.

Since plan 127 each domain runs two **worker lanes** (`RuntimeLane::General` and `RuntimeLane::Latency`), so a runtime generation starts four persistent workers (two per domain) and each lane has its own bounded command mailbox. Lanes are a scheduling device inside a domain, never a trust boundary: both lanes of a domain install the same op set and share the domain's `PackageService`/`PackageLoadEntryAllowlist`. The lane topology, queue bounds, heap restoration, and the trust-domain invariants are documented in [Persistent Runtime Hardening](persistent-runtime-hardening.md#worker-lanes-queue-bounds-and-heap-restoration-plan-127).
...
- Plan 131 deleted `src/server/cross_domain.rs`: its `clay-cross-domain-envelope-v1` validator was never consumed; live cross-domain routing is `dispatch_to_domain` in `src/server/js_runtime/mod.rs`.
- `plans/061-Two-Package-Runtime-Trust-Domains-and-Extension-Authority.md`

### Domain Workers

Each domain has its own `DomainRuntime` struct holding one `LaneRuntime` per `RuntimeLane` (`General`, `Latency`), a monotonic generation counter, and a per-lane `CommandMailbox` reached through a `RuntimeCommandSender` (plan 127 replaced the single unbounded `mpsc` sender). Both lanes share the domain's `ClayOpState`, `PackageService`, and `PackageLoadEntryAllowlist` via `Arc`. The trusted general lane (`clay-js-runtime` thread) evaluates user configuration and trusted package load entries; the third-party general lane (`clay-js-runtime-third-party` thread) evaluates only approved third-party package load entries; the latency lanes (`clay-js-runtime-latency`, `clay-js-runtime-third-party-latency` threads) serve completion and language-intelligence requests for providers registered with a `moduleSpecifier`.
...
The `op_clay_packages_load_in_package_domain` bridge op receives a validated third-party record from the trusted domain, dispatches an `Evaluate` command to the third-party **general** lane, and absorbs the returned `ClayRuntimeEvaluation` registrations (parse handlers, completion providers, language intelligence providers, document analyzers, SDUI/decoration/diagnostic sets) into the trusted general lane's `ClayOpState`. Runtime records are merged; command/mode/UI registries remain per-domain.
...
When a lane is poisoned (timeout, heap limit, or mailbox closure), `dispatch_to_domain` replaces only that lane's worker; when the poisoned lane belongs to the third-party general lane, the cross-domain bridge sender is rewired and `replay_third_party_domain` re-dispatches `Evaluate` commands for all enabled third-party packages. This restores provider registrations without server restart, and latency lanes (which never ran package `Evaluate` commands) are simply restarted empty next time a `moduleSpecifier` provider is needed. Tokens are deterministic (`{apiPrefix}:{providerId}:{index}`), so replayed registrations match existing coordinator entries. Parse, completion, and language-intelligence ops all call the single `ops::registration_token` formatter; its unit test pins that identity shape.

On trusted-domain config reload, `production_reload(current_service)` creates a fresh trusted `DomainRuntime` (both lanes) while cloning the third-party `DomainRuntime`, `PackageService` Arc, and `PackageLoadEntryAllowlist` Arc and re-attaching the surviving third-party workers to the new host channels via `wire_domain_lanes` — so third-party packages, registrations, and language-server sessions survive config evaluation. Only trusted lanes are torn down; third-party lanes are never replaced on config reload.

`wire_runtime_publishers` is the single wiring point for every per-lane host channel (editor/shell/caret publishers and, since plan 130 A1, the server-owned `AgentHostHandle`), which is why both initial construction and a replacement worker route through it: a lane comes up with exactly the authority the server wired into it. The agent-side details — fail-closed hostless lanes, the per-lane registration queue and its hand-over — are in [Agent Process Manager](agent-process-manager.md).

### Document-Analysis Workers

Document-analysis workers are dynamically forked from the owning domain general lane's `ClayOpState` via `new_document_analysis_worker`. They share the parent's `PackageService` and `load_entry_allowlist` but are NOT additional persistent runtimes. A timeout or heap limit poisons the owning lane (not just the analysis worker), so only that lane's worker is replaced and the domain's other lane keeps serving.

### Invariants

- Two trust domains at steady state, each with two persistent `JsRuntime` lanes (general + latency): four workers per generation. Analysis workers are temporary and do not increase the persistent runtime count.
- Lanes never change the trust picture: within a domain both lanes install an identical op set (`lanes_share_their_domain_op_set`), third-party lanes never gain trusted-only ops (`third_party_lane_denies_trusted_ops`), and no V8 object, function, global, or module instance crosses a domain boundary. Lanes only add isolates *inside* a domain; the domain stays the trust boundary.
- Command admission for a package-owned provider is checked against the package's enabled name and exact version before any lane runs it, so revocation fails closed identically in both lanes and in both domains (`revoked_package_commands_refused_per_lane`).
...
- Approval gating is durable: third-party packages require a `PackageApprovalRecord` persisted at `~/.clay/packages` before evaluation. No code executes before approval.
- Two-runtime overhead vs one-runtime: ~5 MiB RSS, 2 extra threads, warm evaluation median < 265 μs (within task 1 baseline). The plan 127 lane split adds one more worker per domain with the latency lane capped at `JS_RUNTIME_LATENCY_LANE_HEAP_LIMIT_BYTES` (32 MiB).
...
`ClayJsRuntimeService` starts the domain lanes' dedicated worker threads when constructed: two per domain (general + latency) via `start_domain_runtime`/`RuntimeLane::ALL`. Facade source is compiled into the binary from `runtime/js/*.js` by `src/server/facades.rs`; loading performs only a static 23-row lookup and never reads or transpiles facade files at runtime. Adjacent `*.d.ts` files are declarations only. Each worker owns one `deno_core::JsRuntime`, one single-thread Tokio runtime for driving `run_event_loop`, one mutable `ClayModuleLoader`, and one shared `ClayOpState`. Public async methods (`evaluate_controlled_module_for_document`, `load_configuration_from_root_with_workspace`, and `load_default_configuration_with_workspace`) send `RuntimeCommand::Evaluate` requests over the target lane's mailbox and await a oneshot response; the caller never holds or shares the V8 runtime.
...
For Phase 13 configuration use, the runtime op state can share the server's `WorkspaceState`. `IpcServer` normally evaluates `~/.clay/init.js` when present; development smoke can instead set `ServerConfig::configuration_root` through `cargo run -- smoke-gui --config-fixture runtime-sdui`, which points the child server at `tests/fixtures/configuration/runtime-sdui/init.js`. `clay:documents` ops call the existing Phase 9 `open_existing_file`, `save_document`, `reload_document`, `document_metadata`, and `list_documents` helpers, while `clay:workspace` lists configured root metadata. Results are serialized as facade JSON with string IDs and sanitized workspace-relative paths. Workspace errors are converted through `WorkspaceError::diagnostic`, so traversal, unknown roots/documents, invalid UTF-8, dirty reloads, stale saves, and IO failures remain typed server validation failures instead of raw filesystem access.
...
The runtime is created with a server-owned V8 heap limit from [`JS_RUNTIME_HEAP_LIMIT_BYTES`](../../../src/perf/budgets.rs) using `v8::Isolate::create_params().heap_limits(...)` through `deno_core::RuntimeOptions::create_params` (latency lanes use `JS_RUNTIME_LATENCY_LANE_HEAP_LIMIT_BYTES`, the smaller of the two). A near-heap-limit callback records that the heap guard fired, calls `terminate_execution()`, and lets Rust map the failure to `ClayRuntimeError::HeapLimit` / `runtime.heap_limit` with sanitized text. The raised limit is a temporary stop-gap: every non-poisoned evaluation ends by restoring the configured limit and cancelling any pending termination request, so the cap cannot ratchet upwards. The heap limit is not exposed through `init.js` or any Clay configuration API.

Timeout and heap-limit termination are treated as runtime-lane poisoning. The lane sends the sanitized diagnostic for the failed command, then exits instead of reusing the isolate. The next command for that lane lazily starts a fresh worker, so stale `globalThis` state and half-mutated module state are not reused, while the domain's other lane keeps running. JS parse-handler tokens are not replayed into the fresh worker; server generation reload and `ParseCoordinator` generation checks remain responsible for replacing handlers and ignoring stale parse work.
