...
`TransientMenuItem` holds a display label, optional detail text, accessibility label, provenance (`BuiltIn` or `Package { name, version }`), and an inert `TransientMenuAction`. Item labels are capped at `MAX_LABEL_CHARS` (128), details at `MAX_DETAIL_CHARS` (256), and accessibility labels at `MAX_ACCESSIBILITY_LABEL_CHARS` (256). No item carries callbacks, native handles, raw CSS, raw op names, or executable code.
...
Activation from the overlay or from keyboard handlers produces a `TransientMenuAction`. The server normalizes that action into a `CommandExecutionRequest` and routes it through `CommandExecutor` from `src/server/command_execution.rs`. This keeps command validation, permission checking, provenance verification, and side effects in one server-owned path.
...
- Items carry only inert command IDs and bounded JSON arguments.
- No callbacks, raw ops, native handles, raw CSS, renderer callbacks, filesystem paths beyond sanitized labels, credentials, or executable package code may be stored in session items.
