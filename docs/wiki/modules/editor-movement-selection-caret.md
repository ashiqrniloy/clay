...
- **Gate** (`src/server/ops/editor.rs::require_editor_control`, every editor op): package caller → approved `editor-control` AND active major mode ∈ declared modes; trusted caller without package context (user configuration) → allowed. Active mode resolves from the active manifest's document scope + mode registry on the trusted worker, and from a host-replicated snapshot (`RuntimeCommand::UpdateActiveEditorMode`) on the third-party worker, which holds no mode registry of its own.
