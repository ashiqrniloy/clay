...
Clay evaluates a constrained local configuration entry point from a configuration root containing `init.js`. Server startup loads the default `~/.clay/init.js` when it exists, and tests can supply explicit fixture roots. While running, a bounded polling task watches the effective root and delegates changes to the serialized runtime reload service. The runtime supports the documented `clay:configuration` facade, local relative `.js` modules under the same configuration directory, and read-only configuration state for the entry point, loaded modules, and validated package-option records.

## Responsibilities

- Treat `~/.clay/init.js` as the default user configuration entry point while allowing tests to provide an explicit configuration root.
...
- **No configuration keys were added.** Watcher tuning, design-system
  ...
  (`src/server/runtime_generation_tests.rs` / `src/server/js_runtime/tests/`).
...
- `ClayJsRuntimeService` and `IpcServer::reload_runtime_generation` (`src/server/js_runtime/mod.rs`, `src/server/runtime_reload.rs`) already evaluate fresh candidates, serialize attempts, validate before commit, and preserve the active generation on failure. A watcher delegates to this service; it does not add IPC, generation, lock, or package logic. Its effective root must match configuration loading: explicit `ServerConfig.configuration_root` wins, otherwise the existing default-root resolver watches `~/.clay` when `init.js` exists.
...
Implemented generic additions are: bounded module-error collection/drain for optional configuration imports, a cancellable bounded config-root polling/debounce task, and reusable live runtime-diagnostic fanout. Polling stays off keypress, edit acknowledgement, paint, layout, and parse paths. Optional loading and watching retain the existing deny-by-default authority model: canonical root containment remains mandatory, and no filesystem, network, shell, package, workspace, AI, WASM, native-widget, raw-op, or client-JavaScript authority is added. Optional-module coverage lives in `src/server/js_runtime/mod.rs` and `src/server/configuration.rs`; watcher coverage lives in `src/server/config_watch.rs` and `src/server/runtime_generation_tests.rs`, while live-fanout coverage is in `src/server/connection/mod.rs`.

## Phase 20.6 persisted preferences and precedence

Phase 20.6 adds `PersistedPreferences` in `src/server/configuration.rs`: a closed `~/.clay/preferences.json` store with at most four keys (`theme`, `appearance`, `typography`, `designSystem` — the last added by Plan 102 as a validated non-empty specifier string), bounded to `PREFERENCES_PAYLOAD_BUDGET_BYTES` (8 KiB), validated at load and persist time, and authority-rejecting. `load_preferences` skips `null` fields and drops corrupted/oversized/manual-edit fields field-by-field with a diagnostic so startup never breaks. `persist_preference` writes atomically (tmp + rename); `clear_preferences` backs `settings.reset`. The `setPackageOption` source taxonomy is extended with `ui-session` to label these persisted values, but no new `clay:configuration` export is added — appearance is a `clay:theme` API (`theme.setAppearance`), and the `clay:configuration` module stays closed.

A single documented precedence applies on every startup/reload (highest wins): `ui-session` (`preferences.json`, written by `settings.setTheme`/`settings.setAppearance`) > `init-js` (`init.js` `setTheme`/`setAppearance`/`setTypography`) > canonical/package default (appearance-derived Modus default or Clay core default). `apply_persisted_preferences` runs in the `src/server/js_runtime/mod.rs` harvest immediately after `init.js` evaluation, so a UI choice always overrides the equivalent `init.js` call. Canonical-default resolution (Modus Operandi/Vivendi) also runs in the harvest when no explicit theme was set. Phase 20.6 implementation history: `docs/wiki/archive/phase20.6-theme-segregation-settings-ui.md`.

### Theme packages, appearance, and the settings surface

**Canonical theme packages.** `@clay/theme-modus-operandi` (light default) and `@clay/theme-modus-vivendi` (dark default) mirror the Gruvbox package structure: inert `clay.contributions.textStyles` (48 entries each), no-op ESM entries, faithful upstream Modus palettes (Protesilaos Stavrou v4.6.0, GPL-3.0-or-later; `keyword_bold: false` unlike Gruvbox). `src/packages/bundled.rs` registers bundled packages as `BundledPackageEntry` rows (root, name, version, FNV-1a-64 `fingerprint`), directory-scan embedding, alphabetical order; `@clay/settings` is registered the same way.

**Appearance resolution.** `src/protocol/mod.rs` defines `Appearance { Light, Dark, System }` (default `System`; `System` → dark/Vivendi when no OS signal is available) and `ResolvedAppearance`. `src/server/ops/theme.rs` provides `canonical_default_specifier` (Light → Operandi, Dark → Vivendi), `resolve_canonical_default_theme` (bundled-inventory `ActiveTheme` build with no `loadPackage` call), `op_clay_theme_set_appearance` (bounded enum, `theme.invalid_request` on unknown), and the `pub(crate)` shared primitives `apply_theme`/`apply_appearance`/`apply_typography` used by both the ops and the harvest. `ClayOpState` carries `appearance` + `explicit_theme_active`; an explicit `setTheme` always wins over the appearance-derived canonical default. An absent `init.js` loads no runtime theme (existing error contract preserved).

**`@clay/settings` panel.** First-party fixed right-slot `panel` (not a transient `modal`: the ~3.4 KiB tree exceeds `SDUI_UPDATE_PAYLOAD_BUDGET_BYTES` 1024) composed only of catalog kinds — `panel`+`scroll`+`flex`, `collapse` sections, `dropdown` pickers (theme/appearance), `textInput` (font stacks, sizes/hierarchy ratios, `validationState`), `label`, `button` — emitting inert `settings.*` command intents (`setTheme`, `setAppearance`, `setTypography`, `reset`, `open`, `close`). `src/server/connection/runtime.rs::sdui_command_request` forwards `SduiActionSource` (`ListItem.item_id` / node id) as command arguments.

**Settings command validation and persistence.** `src/server/command_execution.rs::execute_settings`: `settings.setTheme` requires a bundled first-party `@clay/theme-*` specifier; `settings.setAppearance` requires `light`|`dark`|`system`; `settings.setTypography` takes one complete three-profile/seven-ratio JSON transaction validated by the shared typography parser before atomic persistence and again on reload. `persist_settings_change` (`PersistOutcome`) merges `setTheme`/`setAppearance`/`reset` into `preferences.json` (atomic tmp + rename) and triggers `reload_runtime_generation()` — persist → reload → `init.js` re-eval + `apply_persisted_preferences` → `RuntimeStateSnapshot` fanout; no restart, no separate theme broadcast primitive.

**Theme trio API (Plan 088 verification).** `theme.setTheme` / `theme.setAppearance` / `theme.setTypography` are the complete user-facing theme API (`runtime/js/theme.js` facades, ops `op_clay_theme_set_theme`/`set_appearance`/`set_typography`, refs `docs/reference/clay-js-api/theme/{set-theme,set-appearance,set-typography}.md`). `clay:configuration` stays closed; appearance is a `clay:theme` API. Invariants: explicit `setTheme` beats the canonical default; `System` → dark fallback; `preferences.json` closed-key/authority-rejecting; theme packages are inert data (no renderer code, raw ops, file/network/shell access); invalid typography transactions preserve the previous preference and generation. Tests: `tests/theme_packages.rs` (inert 48-entry mappings, distinct palettes, AA contrast), `src/server/js_runtime/mod.rs` (canonical-default + preferences precedence matrix), `src/server/ops/theme.rs`, `src/server/configuration.rs`, `src/server/command_execution.rs`, `tests/clay_js_doc_registry.rs::configuration_api_documents_phase20_6_appearance_and_precedence`.
...
- [Protocol and Performance Pattern](../../../.agents/skills/clay-execution/references/protocol-perf.md)
