...
Implementation targets are generic: `runtime/js/ui.js` or `runtime/js/input.ts`, `src/server/ops/ui.rs` or `src/server/ops/input.rs`, `src/server/ui.rs`, `src/shell/components.rs`, `src/shell/package_ui.rs`, `src/masonry_sdui.rs`, `src/packages/record/mod.rs`, and `src/packages/conflict.rs`. Do not add raw native event callbacks, package-owned pointer handlers, client-side JavaScript, or Masonry event branches.
...
Selected public API target: `ui.serverSetLayoutOverride` / `serverSetLayoutOverride`.
...
Selected public API target for package options: `configuration.setPackageOption` / `setPackageOption`.
...
- Do not promote `ui.serverRegisterUiStateScope`, `ui.serverSetLayoutOverride`, or `configuration.setPackageOption` by wiring only inventory rows or raw ops. Public callable APIs require facade, op wrapper, server validator, reference docs, docs-index links, generated registry coverage, API inventory metadata, tests, and Rust visibility mapping. `ui.serverRegisterInputContribution` and `ui.serverRegisterUiStateScope` now satisfy that promotion checklist.
...
2. Add generic input/action/focus metadata only as bounded inert declarations. Phase 18.4 selected and implemented `PackageInputContribution` / `ui.serverRegisterInputContribution` as the standalone API target.
3. `PackageUiStateScope` is promoted through `ui.serverRegisterUiStateScope` as an inert declaration primitive; add runtime storage only for implemented shell/UI state and mark persistence/mutation semantics explicitly implemented or deferred.
4. Promote `PackageLayoutOverride` through `ui.serverSetLayoutOverride` only for supported user/package overrides with deterministic precedence and validation.
5. Promote `PackageOwnedConfiguration` through `configuration.setPackageOption` only for package-declared typed options and hidden-key rejection diagnostics.
...
- Command: `cargo test --test protocol --test security --quiet`

## Related

- [Primitive Architecture](../modules/primitive-architecture.md)
...
- [Slot-Aware Package UI](../modules/slot-aware-package-ui.md)
- [Masonry Shell Runtime](../archive/masonry-shell.md)
- [Server-Driven UI Protocol Schema](../modules/server-driven-ui.md)
- [Command Registry](../modules/command-registry.md)
- [Package Loading](../modules/package-loading.md)
- [Configuration Runtime](../modules/configuration-runtime.md)
