...
1. Add generic parse-window snapshot and parse-policy support around `src/protocol/parse.rs`, `src/server/parse_coordinator.rs`, `src/server/document.rs`, `src/server/ops/parse.rs`, and `runtime/js/parse.js`.
2. Add generic syntax/decor cache budget and chunk metadata around `src/protocol/decorations.rs`, `src/server/decorations.rs`, `src/client/mod.rs`, `src/editor/surface/mod.rs`, and `src/perf/budgets.rs`.
