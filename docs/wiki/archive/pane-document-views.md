...
> React workspace: [React CodeMirror Editor](../modules/react-codemirror-editor.md) and
> [React Tabs, Splits, and Layout Persistence](../modules/react-tabs-and-splits.md).
...
- `src/server/behavior.rs`, `src/server/ops/mod.rs` — per-document behavior manifest layers (see [Behavior Manifests](../modules/behavior-manifests.md))
...
- Closing a dirty pane is blocked until the save-conflict menu resolves; clean closes release active + retained leases through the server's capability-gated close path. Plan 086 manual verification still found a follow-up focus/accessibility panic during the dirty-close path (`Focused ID #4 is not in the node list`); see [Plan 086 release integrity](../modules/plan086-release-integrity-and-accessibility.md).
...
[Repeatable UI Review Harness](../modules/ui-review-harness.md)); interactive completion
...
- [Multi-Document Sessions](../modules/multi-document-sessions.md) — session store mechanics per pane
- [Behavior Manifests](../modules/behavior-manifests.md) — per-document manifest layers
- [Masonry Shell Runtime](../archive/masonry-shell.md) — pane hosts, focus actions, routing targets
- [Masonry Editor Widget Status Observability](../archive/masonry-editor.md) — connection chrome responsibilities
- [Server Document State](../modules/server-document-state.md) — server lease/duplicate authority
...
- [Repeatable UI Review Harness](../modules/ui-review-harness.md) — plan 087 fixture/capture workflow (welcome and completion states)
