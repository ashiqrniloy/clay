...
| `TreeSitterSyntaxHandler` | Generic grammar/query/vocabulary handler in `src/server/syntax/mod.rs`; one parser mutex per registered grammar handler, per-document cached tree, query timeout, complete capture shaping, and decoration validation. | Keep one generic handler, first-party descriptor data, shared capture-to-`TokenType`/`Modifiers` mapper, parser/query budgets, and server-side execution. | Implemented: exact incremental reuse, changed/visible querying, and complete capture fan-out into stable 128-byte sets without a 32-span truncation cap. |
...
`replacement_ranges` in `src/server/syntax/mod.rs` converts affected (changed+invalidated) ranges into a shared 128-byte UTF-8-safe replacement-chunk grid. The handler queries the full envelope covering every touched replacement chunk, clips captures at exact chunk boundaries, and constructs `DecorationSet` members from the same grid — so query coverage and replacement coverage are identical. Only chunks that intersect the query envelope are published; untouched adjacent chunks are never emitted.
...
- [Parse Task Lifecycle](../modules/parse-task-lifecycle.md)
- [Decoration Transport](../modules/decoration-transport.md)
- [Syntax Grammar Registry](../modules/syntax-grammar-registry.md)
- [Masonry Editor](../archive/masonry-editor.md)
