...
> [Tauri/React Desktop Cutover](../modules/tauri-react-cutover.md) and
> [React CodeMirror Editor](../modules/react-codemirror-editor.md) for current ownership.
...
Caret/selection/diagnostics stay on `StyleRegistry` (`base.caret`/`base.selection`/`diagnostic_style`); no new `BaseUiColorKey`. See [Phase 20.4 Core Component Uplift](../archive/phase20.4-core-component-uplift-primitive-review.md) and [Server-Driven UI Protocol Schema](../modules/server-driven-ui.md).

## Plan 071: movement, multi-cursor, caret, text objects

Plan 071 extends the widget and surface with first-class editing primitives; see [Editor Movement, Selection, Caret, Ligatures, and Text Objects](../modules/editor-movement-selection-caret.md) for the full implementation page. Widget-level facts:
...
Phase 22.2 (2026-08-05) splits the widget: `EditorWidget` keeps everything that is **per-connection** — SDUI native state, `PackagePanelHost`/`PackageOverlayHost` children, shell-preferences, `RuntimeStateSnapshot` validation, the master `ClientEditQueue` (`edit_queue_shared()` hands out clones) — and delegates all **per-document** editing state to a `PaneDocumentView` child for pane 1 (other panes host their own views). See [Pane Document Views](../archive/pane-document-views.md) for the full model; widget-level facts:
...
Client Views](../modules/tabs-and-clients.md)). `EditorAction` carries `DriverSession`
...
- [Tabs and Independent Client Views](../modules/tabs-and-clients.md)
...
- [Editor Movement, Selection, Caret, Ligatures, and Text Objects](../modules/editor-movement-selection-caret.md)
