...
- `src/masonry_sdui.rs::SduiNativeState::apply_snapshot` and `apply_update` remain the SDUI panel/status application path; the validated native UI tree is rendered by a retained reconciled Masonry subtree (`SduiRegionWidget`, `src/masonry_sdui_region.rs`) hosted as a child of `EditorWidget` (see [SDUI / Package-UI Retained Masonry Reconciliation](../archive/masonry-sdui-region.md)).
