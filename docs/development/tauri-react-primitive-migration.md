...
| Tabs / splits / panes | `src/server/connection/mod.rs` tab registry, `src/shell/layout/mod.rs`, `src/masonry_shell/*`, `src/driver/*` | TabRegistryBinding, TabBarChrome, WorkingAreaLayout, PaneSplitTree, PaneSlotLayout | reuse + adapter + projection + delete (window chrome) | `frontend/src/shell/{tabs,splits,panes}/*` |
...
| Persistence | `src/server/connection/mod.rs` registry, `src/driver/restore.rs`, `src/shell/layout_persist.rs` | TabRegistryBinding | reuse + projection | `frontend/src/persistence/layout.ts` |
