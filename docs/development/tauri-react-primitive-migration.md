...
| Tabs / splits / panes | `src/server/connection/mod.rs` tab registry, `src/shell/layout.rs`, `src/masonry_shell/*`, `src/driver/*` | TabRegistryBinding, TabBarChrome, WorkingAreaLayout, PaneSplitTree, PaneSlotLayout | reuse + adapter + projection + delete (window chrome) | `frontend/src/shell/{tabs,splits,panes}/*` |
