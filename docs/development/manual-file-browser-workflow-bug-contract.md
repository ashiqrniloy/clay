...
| File-browser actions break after Markdown activation | Open a `.md` file, then click another file or directory | Server reports `UnknownActionCommand("workspace.openFile")` after Markdown behavior install / parse-time diagnostic | server `StaticSduiState` workspace-browser validation |
| Parse timeout poisons workflow instead of degrading | Open Markdown and wait for open-time follow-up | UI shows `parse.open_activation_timeout` / "Open-time parse did not finish before the decoration freshness deadline." and subsequent navigation stops working | open-document follow-up diagnostics and server SDUI validation |
