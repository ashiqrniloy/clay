...
| `frontend/src/coding-agent` (agent panel; `frontend/src/agent` owns the transport) | AG-UI message snapshot, run status | `AbstractAgent` transport over Tauri channel; submit/cancel/steer intents to declared commands | Transcript/composer presentation; composer interaction is local-only | Credential-free events; inert tool payloads | Abort cancels run; unsubscribe drops channel |
