...
| `commandCentre` | `scrim`† | `default` | `rest` | `CommandPalette` (`frontend/src/command-centre/CommandPalette.tsx`) veil (the slot itself is retired by plan 125; the veil reads `modal.scrim`) | `[data-entering]`, `[data-exiting]` | Material (backgroundOpacity, backdropBlur) | Layout-neutral | `surface.scrim`, `opacity.scrim` | `core.commandCentre.scrim` | Veil behind the composer palette and `@` mentions menus (working area + rail, never the lane); the retired centered dialog shared it |
| `commandCentre` | `dialog`† | `default` | `rest` | — (slot retired by plan 125: the sheet is the field's menu, not a `Dialog`) | None | Geometry, Material, Borders, Shadows | Mixed | `surface.overlay`, `border.strong`, `elevation.overlay` | `core.commandCentre.dialog` | Retired (plan 125): the `Dialog` role/`aria-modal` belonged to the deleted centered host — the palette is a modeless sheet as wide as its composer box, with no dialog role |
| `commandCentre` | `input`† | `default` | `rest`, `focus` | — (slot retired by plan 125: the query is the composer field itself) | `[data-focused]` | Geometry, Material, Borders, Outlines | Mixed | `surface.control`, `text.primary`, `border.focus` | `core.commandCentre.input` | Retired (plan 125): the query is the composer field itself; server-owned results are unchanged |
| `commandCentre` | `listBox`† | `default` | `rest` | `CommandPalette` `.palList` scrolling region (the slot itself is retired by plan 125) | None | Geometry (max-height), Material | Layout-affecting | `surface.overlay` | `core.commandCentre.listBox` | `role="listbox"`; capped at 256 items (the palette's own scrolling region carries this contract now) |
| `commandCentre` | `item`† | `default` | `rest`, `hover`, `active`, `focus`, `selected` | `CommandPalette` rows via `list.default.row.*` (the slot itself is retired by plan 125) | `[data-hovered]`, `[data-focused]`, `[data-selected]` | Geometry, Material, Borders | Mixed | `surface.list`, `surface.hover`, `surface.selected`, `text.primary` | `core.commandCentre.item` | `role="option"`; typeahead navigation (the palette's rows carry this contract through `list.default.row.*` now) |
| `commandCentre` | `status` | `default` | `rest` | `CommandPalette` `<output>` count line | None | Foreground (textColor), Typography | Layout-neutral | `text.muted` | `core.commandCentre.status` | `typography.status`; live result count |
| `commandCentre` | `empty` | `default` | `rest` | `CommandPalette` empty state | None | Geometry (padding), Material, Borders | Mixed | `surface.overlay`, `border.subtle`, `text.muted` | `core.commandCentre.empty` | Empty query results view |
