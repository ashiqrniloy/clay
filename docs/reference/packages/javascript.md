...
`@clay/javascript` is Clay's first-party grammar-only JavaScript syntax highlighting package. Its source scaffold lives under `packages/javascript/`.
...
The package is explicit opt-in and is not auto-loaded. Without this line, `.js`, `.jsx`, `.mjs`, and `.cjs` files remain editable under `core.code` with no JavaScript syntax grammar.
...
The package declares no major mode, no commands, no completions, SDUI, UI panels/components/overlays, key routing, text transforms, behavior manifests, configuration, theme tokens, layout overrides, or package options. Syntax grammar selection is separate from active major mode, so JavaScript files can stay `core.code` while using the JavaScript grammar for decorations.
