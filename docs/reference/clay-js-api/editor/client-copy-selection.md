...
agent_guidance: Use `editor.clientCopySelection` only as a documented command ID for `bindKey`; do not expose raw clipboard text APIs, server/package clipboard access, raw Rust calls, protocol DTOs, or `Deno.core.ops`. Prefer `clientCutSelection` / `clientPasteClipboard` for cut/paste chords.
...
Use this API when a user wants to bind an alternate copy chord in `~/.clay/init.js`.
...
```ts
bindKey("Ctrl+Shift+C", "editor.clientCopySelection", { scope: "editor" });
```
...
Returns the string literal command ID `"editor.clientCopySelection"` synchronously. The helper does not touch the clipboard, call the server, execute package code, mutate document text, read files, or run client-side JavaScript.

## Errors

The helper has no runtime errors. `bindKey` can reject malformed key chords, unsupported scopes, unsupported `when` clauses, or undocumented command IDs. The native command path can report a sanitized `client.clipboard.write_failed` diagnostic if the OS clipboard backend fails.
...
- Stable ID: `editor.clientCopySelection`
