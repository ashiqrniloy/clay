---
id: clay.shell.clientSplitPaneDown
kind: clay-js-api
...
agent_guidance: "Use `clay.shell.clientSplitPaneDown` only as a documented command ID for `bindKey` to name the down-split direction. It is an alias of `clay.shell.clientSplitPaneHorizontal` (canonical ID, unchanged default `Ctrl+-` chord); prefer the canonical ID in new configuration. Avoid raw Rust calls, protocol DTOs, or `Deno.core.ops`. Pane topology mutation is Clay-owned; packages interact through inert `serverRequestLayoutIntent` only."
...
`clientSplitPaneDown` is the public Clay JS API descriptor for **Split Pane Down** (Phase 22.7 alias). It returns the stable command ID `clay.shell.clientSplitPaneDown` so configuration, help, key-binding discovery, and agents can name the direction without hard-coding Rust shortcuts.

The alias resolves to the existing `SplitPaneHorizontal` handler — the same stacked layout (`SplitOrientation::Vertical`) as `clay.shell.clientSplitPaneHorizontal`: the focused pane keeps its top half and a new pane occupies the bottom half. The pane cap is `MAX_PANES_PER_TAB = 4`; the split is a no-op at cap.
...
Use this API when a user wants to bind a direction-named pane-management chord in `~/.config/clay/init.js`. New configuration should prefer the canonical `clay.shell.clientSplitPaneHorizontal`; the alias exists for direction-named bindings and help discovery.
...
The alias carries no default chord; the canonical `Ctrl+-` binding for `clay.shell.clientSplitPaneHorizontal` is unchanged.
...
Default: none (alias — the canonical `clay.shell.clientSplitPaneHorizontal` keeps its `Ctrl+-` default). Additional bindings may be configured with `bindKey`.
...
Returns the string literal command ID `"clay.shell.clientSplitPaneDown"` synchronously. The helper does not touch the shell, call the server, execute package code, mutate document text, read files, or run client-side JavaScript.
...
Use `clay.shell.clientSplitPaneDown` only as a documented command ID for `bindKey` to name the down-split direction. It is an alias of `clay.shell.clientSplitPaneHorizontal` (canonical ID, unchanged default `Ctrl+-` chord); prefer the canonical ID in new configuration. Avoid raw Rust calls, protocol DTOs, or `Deno.core.ops`. Pane topology mutation is Clay-owned; packages interact through inert `serverRequestLayoutIntent` only.
...
- Backing Rust/current owner: `src/masonry_shell.rs::ClayShellWidget::apply_shell_client_command` (alias routed in `ShellClientCommand::from_command_id`); `src/shell/layout.rs::PaneSplitTree`

## Lookup metadata

- Stable ID: `clay.shell.clientSplitPaneDown`
