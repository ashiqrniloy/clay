---
id: shell.clientResizePaneRight
kind: clay-js-api
...
agent_guidance: "Use `shell.clientResizePaneRight` only as a documented command ID for `bindKey` to remap the default Phase 22.1 pane-management chord. Avoid raw Rust calls, protocol DTOs, or `Deno.core.ops`. Pane topology mutation is Clay-owned; packages interact through inert `serverRequestLayoutIntent` only."
...
`clientResizePaneRight` is the public Clay JS API descriptor for **Resize Pane Right**. It returns the stable command ID `shell.clientResizePaneRight` so configuration, help, key-binding discovery, and agents can name the route without hard-coding Rust shortcuts.
...
```ts
bindKey("Ctrl+Alt+Shift+Right", "shell.clientResizePaneRight", { scope: "global" });
```
...
Returns the string literal command ID `"shell.clientResizePaneRight"` synchronously. The helper does not touch the shell, call the server, execute package code, mutate document text, read files, or run client-side JavaScript.
...
Use `shell.clientResizePaneRight` only as a documented command ID for `bindKey` to remap the default Phase 22.1 pane-management chord. Avoid raw Rust calls, protocol DTOs, or `Deno.core.ops`. Pane topology mutation is Clay-owned; packages interact through inert `serverRequestLayoutIntent` only.
...
- Stable ID: `shell.clientResizePaneRight`
