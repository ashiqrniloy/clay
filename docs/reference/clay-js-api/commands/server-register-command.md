...
- **Control Center** — the Control Center (`src/server/control_center.rs`, `pub(crate)`) is a built-in composer-anchored command-palette workflow reached through the built-in server-first command `controlCenter.open` — shipped with the Plan 124 default `Ctrl+X Ctrl+O` chord (Global, `ServerFirst`) and overridable via [`keybindings.bindKey`](../keybindings/bind-key.md); it has no callable Clay JS facade of its own. The separate `shell.toggleAgentLane` command-ID helper owns the `Ctrl+X Ctrl+P` lane-visibility route.
- **Path Browser** — the Phase 24.3 Path Browser (`PathBrowserSession` in `src/shell/path_browser.rs`, `BuiltInUserBrowseListing` in `src/server/workspace/mod.rs`, both `pub(crate)`) is a built-in dired-style browse workflow reached through the built-in server-first command `controlCenter.openPath` — shipped with the Phase 24.5 `Ctrl+X Ctrl+F` sequence default (Global, `ServerFirst`, rebindable/removable via `bindKey`/`unbindKey` without changing the id); it has no callable Clay JS facade, no raw arbitrary-path op, no browse/grant/tab-id selector API, and packages cannot open, drive, intercept, or receive paths from it. Browse authority is ephemeral; activation converts it into exactly one `SingleFile` or `Directory` grant.
- **Agent picker stages** — the six built-in server-first picker commands (`agent.clientOpenAgentPicker`, `agent.clientOpenProviderPicker`, `agent.clientOpenModelPicker`, `agent.clientOpenProviderSetup`, `agent.clientOpenSessionPicker`, `agent.clientOpenSessionSearchPicker`) map through `src/server/agent_picker.rs::picker_kind_for_command` onto the Clay-owned `AgentPicker` session (`src/server/agent_picker.rs`, `pub(crate)`). Since Plan 125 every stage renders on the composer's `/` palette (`TransientMenuOrigin::CommandPalette`; see [the Plan 125 authoring contract](../packages/creating-packages.md#plan-125-authoring-contract-one-palette-for-every-picker-and-the-halo)), and the session — prompt, query, rows, stage trail, and the shielded secret stage — stays Clay-owned. The IDs have no callable Clay JS facade, no op, and no inventory entry; calling one through `commands.serverExecuteCommand` resolves the built-in but performs no session work, because a session opens only on a user command intent from the client. Package UI contributions may still name the IDs as action targets (they are built-in Clay commands, not package-registered ones — see [`keybindings/bind-key.md`](../keybindings/bind-key.md#plan-125-picker-command-ids)): the package triggers a Clay-owned session and gains no session, stage, query, or credential access.
