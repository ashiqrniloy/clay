...
| Key binding management | `keybindings.bindKey`, `keybindings.unbindKey`, `keybindings.listKeyBindings` | Configuration API | Future configuration produces inert manifests; keypresses do not run JavaScript. Built-in launch IDs `shell.toggleAgentLane` and `controlCenter.open` are documented bind targets; the latter remains a Clay-owned command session, not a callable menu API. |
| Agent lane visibility | `shell.toggleAgentLane` | Client UI command ID | Synchronous `clay:shell` helper returns a stable ID; `Ctrl+X Ctrl+P` routes through the existing ServerFirst command path, then toggles per-tab shell state locally. No lane configuration property or hot-path operation is added. |
| Composer palette launch | `controlCenter.open` | Built-in server-first command ID | `Ctrl+X Ctrl+O` opens the Clay-owned composer-anchored catalogue; configure through `keybindings.bindKey` only. No standalone `clay:controlCenter` facade, package session API, or authority grant. |
