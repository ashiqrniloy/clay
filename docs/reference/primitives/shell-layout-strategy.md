...
- The SDUI sidebar renders from Clay-owned left-slot geometry through the React SDUI projection; it is not a package slot API. Package panel/component contributions compose through the generic Phase 18.3 runtime state and render as separate projected surfaces.
...
blur, filter, or offscreen render target. Plan 124 re-anchored the command and
path sessions onto the agent lane's composer (`CommandPalette`), so the centered
origin now hosts the agent picker and package UI dialogs; both hosts read the
same generation-stamped catalogue. Completion, context-menu, menu-bar,
...
- The SDUI sidebar uses the internal `PaneSlotLayout` state for fixed left-slot geometry and renders through the React SDUI projection; package panels/overlays compose through the generic Phase 18.3 runtime state into separate projected surfaces.
