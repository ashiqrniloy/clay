...
- The Tauri desktop shell (`src-tauri`) hosts the React application shell; the editor is a CodeMirror host inside the working area instead of being the application shell. (Pre-cutover, the native client started a dedicated shell root widget.)
- `src/shell/layout/mod.rs` owns internal Rust `WorkingAreaLayout` state for one working area, a layout version, the active/root pane, and the editor component binding.
...
- The SDUI sidebar renders from Clay-owned left-slot geometry through the React SDUI projection; it is not a package slot API. The tree carries the region and names the host token that sizes it (`dimension.sidebar.default`), while the **shell** places that region as the working area's own left rail (`hostRailRegionId`/`SduiRegion` in `frontend/src/sdui/renderer.tsx`, `.side` in `frontend/src/shell/workspace-panes.module.css`), so the sidebar runs the working area's full height beside the lane (plan 126, closing plan 125's D7). Package panel/component contributions compose through the generic Phase 18.3 runtime state and render as separate projected surfaces.
- Inert local layout updates and structural observability are internal test/agent inspection surfaces. Observations record layout shape, slot geometry, and component binding without exposing document text, DOM internals, widget handles, raw action authority, raw CSS, raw ops, renderer callbacks, or executable package code.
...
```text
// Current client composition (React), not a package-facing API.
AppShell            // landmarks: header/tabs, main workspace, footer status
└── WorkspacePanes  // per-tab pane/split tree from src/shell/layout/mod.rs state
    └── Pane → slots + ClayEditor (CodeMirror) / package surfaces
```

## Architecture Boundary

Clay owns the package-facing shell vocabulary and compiles validated package declarations into client UI state. Packages do not own native widgets.
...
Package JS declarations and server-side handlers
  ...
  -> Clay-owned React components and CodeMirror rendering
```

The web platform remains an implementation substrate: React Aria Components primitives (`Grid`, `Tabs`, `Dialog`, `ListBox`, disclosure, focus management) plus CodeMirror for the text surface. These names are evidence for implementation feasibility; they are not stable public package APIs. Clay may use built-in primitives or Clay-owned composed components when shell invariants require stricter validation than a generic primitive provides.

Package authors must use Clay concepts: working areas, panes, slots, panels, components, command/action intents, state scopes, and theme tokens. They must not depend on widget/DOM identities, layout pass timing, typed property internals, renderer callbacks, or the shape of Clay's component tree.

## Vocabulary

### Application Shell

The Clay application shell is the Clay-owned root UI composition inside an OS window. In the Phase 18.2 runtime (native client, historical) a dedicated shell root widget hosted the editor as a child component; today the React `AppShell` fills that role and the CodeMirror editor is one content type inside the working area.
...
A `PaneSplitTree` is a Clay-owned tree whose leaves are panes and whose internal nodes are horizontal or vertical splits. Phase 18.2 implemented the internal Rust state and geometry helpers for the one-leaf default and generic split topology; Phase 22.1 added the user-facing split lifecycle (`split_pane` capped at 4 panes, `add_equal_pane`, `close_pane`, `move_pane`, `keyboard_resize`) with multi-pane hosting in the workspace shell; Phase 22.2 added per-pane document views (each pane hosts at most one document of its tab's workspace; the pane↔document mapping is client-local view state); package-facing empty-tab pane-content contribution is public (`ui.serverRegisterPaneContentContribution`); slot-targeted package placement for arbitrary `main` apps beyond empty-tab remains later work.
...
The pane/window layout term means the logical layout inside a Clay window: working area -> pane/split tree -> leaf pane -> slots. It does not grant packages a native window handle, DOM handle, or OS window mutation authority.

### Pane

A pane is a leaf in the pane/split tree. It owns exactly one mandatory `main` container and may own optional `left`, `right`, `top`, and `bottom` panel slots. Pane state may include active component, focus metadata, panel visibility, split ratios, and transient overlay state when later phases implement those fields. Since Phase 22.2 the `main` content of an editor pane is a document view: one editor surface per open document (caret/selection/viewport, undo history, request bookkeeping, and status line; now a CodeMirror instance per pane in the React client).
...
Slots are declarations in the Clay layout model, not renderer containers exposed to packages. Clay validates slot ownership, collision, visibility, and persistence before client state changes are applied.
...
Outside the pane slot model the shell owns one bottom chrome row of the view pane: the persistent **agent lane** (plan 124) — the tab's optional approval strip, composer box (prompt field plus that tab's agent-type/model/effort controls), hint row, and session-environment foot, shared by the tab's Workspace and Agent views and toggled by `shell.toggleAgentLane`. It is shell chrome, not a `PaneSlotLayout` slot, package contribution, `TransientMenuOrigin`, or `OverlayAnchor`: panes and their `bottom` slots end at its top edge, and packages contribute no UI to it. Plan 125 confined it to the view pane — the workspace sidebar and the inspector rail run the full working-area height, and the lane stops at each rail's inner edge, taking the full width only while both are collapsed; plan 126 made the sidebar a rail of that grid too (the tree's token-sized region is host-placed, not a pane column), closing plan 125's open D7 deviation. Its composer is where the `/` palette answers (below), and that sheet is the only transient selection surface left in the shell.

### Transient Panels and Overlays

Transient panels overlay the pane or working area and are dismissible or focus-scoped. Examples include package command palettes, dropdowns, hover documentation, modals, temporary find/replace bars, and menus. The runtime-backed `TransientOverlayContribution` describes package UI as inert data; Clay owns focus trapping, dismissal, z-order, accessibility, and native overlay implementation. The built-in completion projection and the composer palette are separate Clay-owned projections, not package overlay contributions — and since plan 125 the palette is the *only* transient selection surface, the centered one having been retired.

### Bottom Transient Menus and Command Execution

Phase 18.8 specializes the transient panel family with a generic, implemented `TransientMenuSession` for Clay-owned command browsing and picker workflows. A session records prompt text, query text, bounded item metadata, selected index, status text, focus policy, accessibility labels, inert actions, and a stable session ID. Server-routed Control Center/Path Browser sessions are drawn as the composer field's own menu since plan 124 (`TransientMenuOrigin::CommandPalette`: the sheet sits 6 logical pixels above the box, as wide as the box, veiled over the working area and the inspector rail's full height by the field's scrim while the field — the session's query — stays interactive); plan 125 gives the same session the agent picker's stage flow behind a bounded `mode` field (`catalogue`, `path`, `picker`, `secret`, `url`, `oauth`; a shielded in-sheet field for `secret`), so the sheet renders stage prompts, a mode-aware foot, and the stage-back keys from server-owned state — the `secret` stage is the one case where the input moves into the sheet and the composer field is disabled, so a credential never reaches the composer's draft. Completion reuses the retained renderer as a client-local `TransientMenuOrigin::Completion` projection with an IME/caret anchor, modeless focus, and no package-facing anchor. Neither is a fixed bottom panel and neither consumes fixed `PaneSlotLayout` geometry unless a later declaration explicitly installs fixed bottom chrome.

Phase 24.3 adds the Path Browser as a second built-in consumer of the `TransientMenuSession` round trip. `controlCenter.openPath` opens a `PathBrowserSession` (src/shell/path_browser.rs) seeded from the active document's canonical parent, then the bound tab's workspace root, then the server cwd. The session holds a canonical current directory, the editable path input (the server-owned query line — not a package `textInput`), a derived filter fragment, bounded installed entries, persisted selection, and a sticky error status. A listing (`BuiltInUserBrowseListing`, src/server/workspace/mod.rs) runs once per directory-prefix change or empty-filter ascent on Tokio's blocking pool (`spawn_blocking`) with no workspace/tab/menu lock held, capped at `TRANSIENT_MENU_MAX_ITEMS` depth-1 entries sorted deterministically (directories first with an empty filter, fuzzy order otherwise via the shared matcher); filter-only edits perform no filesystem work. Primary activation on a directory descends, on a file opens it (closing the session); Alt+Enter (secondary activation) on a directory opens it as the tab's workspace; Backspace on an empty filter ascends; direct path-bar edits jump to any path (a typed directory needs a trailing separator). Opening a file converts the ephemeral browse authority into exactly one `SingleFile` grant, and a workspace open converts it into exactly one `Directory` root grant for the bound tab only; navigation alone creates no grant, other tabs are untouched, and the native file/folder dialogs remain the fallback capability issuers. The Path Browser session is Clay-owned and server-routed end to end: packages cannot open, populate, intercept, or receive paths from it, and it never runs package JavaScript, blocking IPC, or filesystem work on paint/layout/input paths.
...
No bottom or caret-adjacent transient menu path may run package JavaScript, command handlers, package validation, configuration evaluation, blocking IPC, filesystem, network, shell, AI, WASM, package-manager, package installation, package enable/disable, full-document serialization, raw op, raw CSS, native-widget, or client-side JavaScript work in render/layout/pointer/scroll/key/input handlers. Rendering reads installed inert menu/overlay state only; ordinary editor typing remains client-first. Package-authored menu accessibility labels are normalized once at Clay's host boundary and bounded to the existing 256-character policy.

Phase 24.4 presented built-in command and path sessions as one centered
window-level overlay layer above the shell. `TransientMenuOrigin::Centered`
used cached `dimension.overlay.centered.width` (640 logical-pixel default) and
one `paint_scrim` fill from `surface.scrim`/`opacity.scrim` with no blur,
filter, or offscreen render target. Plan 124 re-anchored the command and
path sessions onto the agent lane's composer (`CommandPalette`), and plan 125
moved the last centered host (the agent picker's stage flow) onto that same sheet and
deleted the projection: `PackageOverlayAnchor` no longer carries a centered
option, `src/shell/package_ui.rs` maps the retired wire value to the bottom
work-area anchor for old frames, and the `commandPalette` origin is the single
transient selection surface, reading the same generation-stamped catalogue.
Completion, context-menu, menu-bar, and package overlays preserve existing local
anchors, and the retired `centered` value is Clay-internal (never a package
`OverlayAnchor` value). The palette reports its rows as a listbox inside the
field's dialog-free sheet — focus stays in the field that holds the query — and
keeps server-owned input containment on the originating pane. A `secret` stage
is the one exception to "focus stays in the field": its shielded input lives in
the sheet and the composer field is disabled for the duration. Plan 088 closes the retained
...
Resolved dimensions are finite, non-negative, and ordered `min <= default <= max`. A missing, non-finite, or misordered override triple falls back to the matching Clay constant tuple per domain before layout — invalid token ordering never constructs a misordered `FixedSlotState`. `FixedSlotState` validation/clamping in `src/shell/layout/mod.rs` runs unchanged on the token-backed values.
...
Resolution occurs at theme/configuration install time. The client caches one `ResolvedUiTheme`; `panel_defaults()` is a cached read over installed state. Native paint, layout, pointer, scroll, keypress, and text-event paths perform no package JavaScript, theme parsing, raw IPC, or re-resolution. Phase 20.3 implemented split-ratio drag, slot resize/collapse interaction, layout persistence (`~/.clay/layout.json`), focus/input routing across splits, and the inert versioned `LayoutIntent` API (`serverRequestLayoutIntent`); these consume the token-backed defaults without redesigning their ownership.
...
The workspace shell hosts one content region per pane leaf. `PaneContent` is `Placeholder | Editor | Document | Package | Welcome`. Empty/new-tab `main` hosts at most one validated package pane-content contribution (`serverRegisterPaneContentContribution`, activation `empty-tab`). No contribution, or two competing contributions, uses the core `Welcome` fallback (Open File / Open Folder only). Open File mounts `Document`. Open Folder binds a workspace root and leaves the entry surface up. A later terminal stays a distinct kind (PTY ≠ SDUI). Do not add product-named variants (`Agent`).

Pane focus is configurable: **click-to-focus** (default) activates a pane on pointer-down; **focus-follows-cursor** activates the pane under the pointer on hover. Editor panes consume pointer-down internally (CodeMirror), so editor focus is derived from the editor's focus state rather than pane-level pointer bubbling. Placeholder panes bubble pointer-down to the shell for direct activation.

Clay owns all topology mutation. The split/close/add-equal/move/resize commands (`shell.clientSplitPaneVertical`, `shell.clientSplitPaneHorizontal`, `shell.clientAddEqualPane`, `shell.clientClosePane`, `shell.clientFocusPaneNext`/`prev`, `shell.clientResizePaneLeft`/`right`/`up`/`down`, `shell.clientMovePaneNext`/`prev`) are `ClientUiCommand`-routed, Clay-owned built-in commands registered in `default_commands()` with `CommandAuthority::ClientUi`. Default keybindings ship in `default_keymaps()` with `KeyBindingContext::Global` (so they fire without editor text focus) and are user-overridable through `keybindings.bindKey` in `~/.clay/init.js`. `route_key` checks `EditorTextFocus` rules before `Global` rules so an editor-scoped binding for the same chord wins.
...
- **Per-pane document view hosting.** The workspace controller remains the connection owner: it holds connection-wide chrome — SDUI sidebar projection, package panel/overlay surfaces, theme/typography/behavior baseline — and routes events to each pane's document view. Every additional pane mounts its own lightweight document view (an independent CodeMirror surface with caret, selection, viewport, undo history, status line, and request bookkeeping). Placeholder panes remain available.
...
Packages still interact with layout only through the inert `serverRequestLayoutIntent` API. No package-facing API opens documents into panes or contributes pane content in Phase 22.2; pane content hosting is internal Clay machinery.
...
Packages must not treat component names as concrete widget types. The same package-facing `Panel` could be implemented by a flex/grid composition or a bespoke component without changing the package contract.
...
| `user-config` | User overrides from `~/.clay/init.js` Clay APIs | Planned |
...
Clay resolves these tokens server-side and installs them as `--clay-*` CSS custom properties and editor styles. Unknown tokens, raw CSS, raw colors without a typed token contract, style strings, and renderer callbacks are rejected.
...
All hidden JSON/TOML/ad hoc layout, style, input, or theme keys are rejected. This includes keys named like `layout.preview.defaultSlot`, `layout.preview.defaultVisibility`, `preview.position`, `preview.defaultVisibility`, or `theme.markdown.heading.1` when they bypass documented Clay JS APIs. User overrides cannot grant filesystem, network, shell, extension loading, AI mutation, workspace mutation, package enable/disable, WASM, raw Deno ops, native widget handles, direct DOM/renderer access, raw CSS, renderer callbacks, client-side JavaScript, unsupported slots, unknown style tokens, unregistered actions, or hidden package state authority.

Configuration evaluation happens at startup, package load, configuration reload, or explicit setting-change time. Render/layout, pointer, scroll, keypress, input handling, and ordinary editor hot paths read already-validated inert state only; they must not run package JavaScript, wait on configuration IPC, parse package metadata, mutate layout from package code, or recompute layout from user JavaScript.
...
Rust visibility audit: Phase 18.2 introduces no new public server-side Rust shell/layout functions. `src/shell/mod.rs` and `src/shell/layout/mod.rs` remain `pub(crate)` internal runtime state; after the cutover they are consumed by the Tauri bridge for layout persistence/restore. The removed native shell root types were deleted with the native client. The behavior-changing shell update and observation helpers (`apply_layout_update`, `observable_snapshot`, `WorkingAreaLayoutUpdate`, `WorkingAreaLayoutObservation`) remain `pub(crate)` and are explicitly not callable from JavaScript.
...
The naming layers are deliberately distinct: the module specifier groups imports, the lower-camel-case export is what JavaScript would call, the stable registry ID is the globally searchable `ui.*` identifier, and the user-facing name is the English help/search label. Raw Rust paths, raw op names, component/type names, protocol DTO names, and generated registry IDs must not become package-facing callable names.
...
| `user-config` | Documented `~/.clay/init.js` Clay JS APIs | default preview slot, panel visibility default, token remap | hidden JSON/TOML/ad hoc keys, permission grants, unknown options |
...
Package UI/layout declarations are validated and applied as inert state updates. Package JavaScript runs server-side during package load, configuration change, explicit command handling, or other documented server-side phases; no package logic runs during render, layout, pointer, scroll, keypress, or input handling. In short: no package JavaScript runs on the client hot path.
...
- direct widget/DOM handles and constructors, widget IDs, layout pass callbacks, or native layout mutation;
- renderer scene/callback authority or direct GPU drawing;
...
Validation failures must become deterministic diagnostics at package load, configuration, or UI update time. They must not panic client rendering and must not silently grant authority.
...
- Internal `WorkingAreaLayout`, `PaneSplitTree`, and `PaneSlotLayout` state exists. Phase 22.1 added user-facing Clay-owned split lifecycle operations (split/close/add-equal/move/resize) and multi-pane hosting; Phase 22.2 added per-pane document views with document-scoped event routing; Phase 22.3 added the server-authoritative in-memory tab registry, the shell-owned tab bar (open/close/switch, dirty-guarded close, reconnect), and per-tab split trees; Phase 22.4 added keyboard tab management (24 tab command IDs + Global default chords, registry reorder ops, bounds/wraparound policies, the driver-owned dirty-close confirm/save flow); Phase 22.5 closed the persistence gap (client-owned versioned `layout.json` v2: tab order, active tab, per-tab workspace + split tree + per-pane documents survive restarts; unsaved edits do not). No public/package-facing API for working-area mutation, split-tree mutation, direct pane-slot layout defaults, tab/tab-bar contribution, or per-tab package chrome is callable yet; empty-tab pane-content contribution is public (`ui.serverRegisterPaneContentContribution`). Other layout interaction stays inert `serverRequestLayoutIntent`. Phase 22.6 completed the window-model accessibility contract (roles/names/announcements — see [Accessibility (Phase 22.6)](../development/accessibility.md)); still-planned tab work: tab-bar keyboard focus traversal (per-card widget focus for AT focus handling), plus multi-client/multi-window persistence (Phase 21).
- The SDUI sidebar uses the internal `PaneSlotLayout` state for fixed left-slot geometry and renders through the React SDUI projection in the shell's own left rail (plan 126); package panels/overlays compose through the generic Phase 18.3 runtime state into separate projected surfaces.
...
- vocabulary coverage for working area, the shell-owned agent lane (plan 124), pane/split tree, pane/window layout, mandatory `main`, optional `left`/`right`/`top`/`bottom`, fixed panels, transient panels, components/elements, action intents, package state scopes, and style/theme tokens;
...
- Substrate boundary wording that treats renderer primitives (layout containers, portals, typed properties, actions) as internal implementation evidence only;
- performance wording that keeps package JavaScript and package parsing out of render/layout/pointer/scroll/key/input handlers;
- security wording that rejects raw CSS, arbitrary client JavaScript, raw `Deno.core.ops`, direct widget/renderer access, renderer callbacks, filesystem/network/shell/AI/WASM authority, and unregistered action targets.
