...
| `SyntaxGrammarContribution` | `parse-document` + `render-decorations` | Phase 18.10 grammar metadata is first-party-only load-time inert package data under `clay.contributions.syntaxGrammars`: package-prefixed grammar ID, language ID, file patterns, package-root-confined `tree-sitter-wasm` artifact path, `.scm` query paths, known capture-to-style-token map, and bounded budgets. Parse/highlight runs as `Background` no-hot-path work bounded by `INCREMENTAL_PARSE_UPDATE_BUDGET_BYTES`, `DECORATION_PAYLOAD_BUDGET_BYTES`, and `SYNTAX_CACHE_BUDGET_BYTES`. Clay rejects non-`@clay/*` grammar packages, absolute/external/traversing paths, native libraries, package-manager/download/shell fields, raw CSS/colors, client JavaScript, raw ops, and duplicate language/pattern registry conflicts. |
