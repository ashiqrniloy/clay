...
| Panels/status/preview UI | `SduiTree` / `SduiTreeUpdate` | Exists/extend | Reuses `src/protocol/sdui.rs`, `src/masonry_sdui.rs`, `src/masonry_sdui_region.rs`, and `clay.sdui.*` APIs | `SDUI_SNAPSHOT_PAYLOAD_BUDGET_BYTES`, `SDUI_UPDATE_PAYLOAD_BUDGET_BYTES` | Server validates inert SDUI trees; client applies snapshots/updates via `SduiNativeState::apply_snapshot`/`apply_update`, which feed a retained reconciled Masonry subtree (`SduiRegionWidget`) hosted as a child of `EditorWidget`. |
