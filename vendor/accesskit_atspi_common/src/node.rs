...
impl PlatformNode {
    ...
    fn resolve_for_editable_text_with_context<F, T>(&self, f: F) -> Result<T>
    where
        for<'a> F: FnOnce(Node<'a>, &Context) -> Result<T>,
    {
        self.resolve_with_context(|node, context| {
            if !node.is_text_input() || node.is_read_only() {
    ...
    /// Replace the entire value of a writable text control.
    pub fn set_text_contents(&self, value: &str) -> Result<bool> {
        self.resolve_for_editable_text_with_context(|node, context| {
            context.do_action(ActionRequest {
                action: Action::SetValue,
                target: node.id(),
                data: Some(ActionData::Value(value.to_owned().into_boxed_str())),
            });
            Ok(true)
        })
    }

    /// Insert text at an AT-SPI character offset.
    pub fn insert_text(&self, position: i32, text: &str, length: i32) -> Result<bool> {
        if length < 0 {
            return Err(Error::IndexOutOfRange);
        }
        let inserted: String = text.chars().take(length as usize).collect();
        self.resolve_for_editable_text_with_context(|node, context| {
            let range =
                text_range_from_offsets(&node, position, position).ok_or(Error::IndexOutOfRange)?;
            context.do_action(ActionRequest {
                action: Action::SetTextSelection,
                target: node.id(),
                data: Some(ActionData::SetTextSelection(range.to_text_selection())),
            ...
            context.do_action(ActionRequest {
                ...
                target: node.id(),
                data: Some(ActionData::Value(inserted.into_boxed_str())),
            });
            Ok(true)
        })
    }

    /// Delete the character range from a writable text control.
    pub fn delete_text(&self, start: i32, end: i32) -> Result<bool> {
        self.resolve_for_editable_text_with_context(|node, context| {
            let range = text_range_from_offsets(&node, start, end).ok_or(Error::IndexOutOfRange)?;
            context.do_action(ActionRequest {
                action: Action::SetTextSelection,
                target: node.id(),
                data: Some(ActionData::SetTextSelection(range.to_text_selection())),
            ...
            context.do_action(ActionRequest {
                action: Action::ReplaceSelectedText,
                target: node.id(),
                data: Some(ActionData::Value(String::new().into_boxed_str())),
            });
            Ok(true)
        })
    }

    pub fn copy_text(&self, _start: i32, _end: i32) -> Result<()> {
        Err(Error::UnsupportedInterface)
    }

    pub fn cut_text(&self, _start: i32, _end: i32) -> Result<bool> {
        Err(Error::UnsupportedInterface)
    }

    pub fn paste_text(&self, _position: i32) -> Result<bool> {
        Err(Error::UnsupportedInterface)
    }
