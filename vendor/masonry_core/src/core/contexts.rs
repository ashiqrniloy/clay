...
// Mutations that detach children need the same focus transfer primitives as
// event handlers. Keeping them on MutateCtx lets container reconciliation move
// focus before it removes the old subtree.
impl MutateCtx<'_> {
    /// Transfer text focus to a surviving widget before a child is detached.
    pub fn set_focus(&mut self, target: WidgetId) {
        trace!("set_focus target={:?}", target);
        self.global_state.next_focused_widget = Some(target);
    }

    /// Clear text focus before detaching a focused subtree with no survivor.
    pub fn resign_focus(&mut self) {
        trace!("resign_focus");
        if self.has_focus_target() {
            self.global_state.next_focused_widget = None;
...
impl_context_method!(MutateCtx<'_>, EventCtx<'_>, UpdateCtx<'_>, RawCtx<'_>, {
    ...
    pub fn remove_child(&mut self, child: WidgetPod<impl Widget + ?Sized>) {
        fn remove_node(
            ...
            // Detaching a subtree must not leave the next accessibility tree
            // focused on one of its widget ids. The old implementation only
            // repaired focus_anchor, so accesskit_consumer received a stale
            // focused ID after pane/tab removal.
            if global_state.focused_widget == Some(state.id) {
                // Keep the old path until run_update_focus_pass can walk its
                // surviving ancestors and clear their focus flags.
                global_state.focused_widget = None;
            }
            if global_state.next_focused_widget == Some(state.id) {
                global_state.next_focused_widget = None;
            }
            if global_state.focus_fallback == Some(state.id) {
                global_state.focus_fallback = None;
            }
