...
fn build_accessibility_tree(
    ...
    if state.is_stashed {
        // Stashed widgets are omitted from parent children lists. Recursing
        // would emit descendant nodes with no parent (accesskit_consumer
        // rejects them) and can leave a stale focused id after pane close.
    ...
    if state.request_accessibility {
...
pub(crate) fn run_accessibility_pass(root: &mut RenderRoot, scale_factor: f64) -> TreeUpdate {
    ...
    // accesskit_consumer panics if `focus` is absent from the node map.
    // Detached widgets can linger in the arena after pane/tab removal, so
    // only advertise a still-parented widget; otherwise park on the window.
    let focus = root
        .global_state
        .focused_widget
