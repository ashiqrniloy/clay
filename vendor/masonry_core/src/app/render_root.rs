...
impl RenderRoot {
    ...
    pub fn handle_access_event(&mut self, event: ActionRequest) {
        ...
        // The accessibility adapter exposes the synthetic top-level Window
        // node, but it is not a Masonry widget. Ignore actions addressed to it
        // instead of dispatching a guaranteed-invalid WidgetId into the event
        // pass (which panics in debug builds).
        if event.target == self.window_node_id {
            return;
        }
