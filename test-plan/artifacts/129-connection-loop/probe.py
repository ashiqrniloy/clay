...
def main(argv):
    ...
    if command == "act":
        ...
        needle = argv[3].lower()
        for node in nodes():
            if str(node.get_role_name() or "").lower() != role:
                continue
            if needle not in str(node.get_name() or "").lower():
                continue
            action = None
            try:
                ...
                action = None
            if action is None:
                print(f"no action interface on {role} {node.get_name()!r}")
                return 1
            names = [
                action.get_action_name(i)
                for i in range(action.get_n_actions())
            ]
            ok = Atspi.Action.do_action(node, 0)
            print(f"action {names[0] if names else '?'} on {node.get_name()!r} -> {ok}")
            return 0
        print(f"no {role} matching {needle!r}")
        return 1
    if command == "set":
        role = argv[2].lower()
        needle = argv[3].lower()
        ...
        for node in nodes():
            ...
            if needle not in str(node.get_name() or "").lower():
                continue
            if "EditableText" not in interfaces(node):
                print(f"{role} {node.get_name()!r} has no EditableText")
                return 1
            Atspi.Component.grab_focus(node)
            Atspi.EditableText.set_text_contents(node, text)
            print(f"set {node.get_name()!r} to {text!r}; {describe_editor(node)}")
            return 0
        print(f"no {role} matching {needle!r}")
        return 1
    if command == "rows":
        print("role|name|states|extents")
        for node in nodes():
            role = str(node.get_role_name() or "").lower()
            if role in {"menu item", "list item", "menu", "list box", "label", "entry"}:
                print(
                    "|".join(
                        [
                            role,
        ...
        return 0
    if command == "mark":
        tail = int(argv[2] if len(argv) > 2 else 120)
        for node in editor_nodes():
            try:
                value = Atspi.Text.get_text(node, 0, -1)
            except Exception:
                value = ""
            visible = value[-tail:].replace(" ", "\u00b7").replace("\n", "\u23ce")
            print(f"chars={len(value)} tail={visible}")
            return 0
        print("no editor node")
        return 1
    if command == "text":
        head = int(argv[2] if len(argv) > 2 else 80)
        tail = int(argv[3] if len(argv) > 3 else 120)
        for node in editor_nodes():
            try:
                value = Atspi.Text.get_text(node, 0, -1)
            except Exception:
                value = ""
            print(f"chars={len(value)} head={value[:head]!r} tail={value[-tail:]!r}")
            return 0
        print("no editor node")
        return 1
