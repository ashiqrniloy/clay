...
| C43 | Restart the daemon after C42, resume both tabs, then list workspace files or submit a follow-up prompt | Each resumed session recovers its recorded root, not the daemon launch directory; A lists A's marker and B lists B's marker. Resume also re-activates the recorded workspace's coding surfaces, because a restarted daemon has neither: the allow-listed MCP servers re-connect and the workspace's graft binding (extension + tools) is re-established — a resumed session must not come up with an empty tool set (plan 130 A2). A dead daemon handle is cleared so the next RPC respawns it rather than hanging. | `tests/agent_session_isolation.rs::real_daemon_serves_one_session_per_workspace`; `clay-agent/src/__tests__/resume.test.ts` (`resumed session binds its tools to the recorded workspace root, not the daemon cwd` — recorded root, graft tool and `mcp:live:` tool after a restart in an unrelated cwd); live protocol probe `test-plan/artifacts/130-agent-host/live-daemon.log` (module 16 A25) |
...
| C59 | Model picker in the lane: open the model trigger, switch provider/model, then submit `/model`; repeat from the palette's `Choose Model` row | Both routes open the daemon model picker **through the composer palette** (plan 125): `/model` and the lane trigger start the same session, which draws as the bottom sheet's `Model` stage with the stage verb `run` — no centered dialog — and the selection persists to the session so the lane's trigger label follows it. With **no** provider configured the trigger is a disabled `Configure a provider` row and a prompt is refused with the configuration error (no silent fallback); with a configured provider whose models inventory is missing, the lane shows the muted provider/model caption instead of a dead dropdown | `AgentLane.test.tsx` (model trigger states; `/model` opens the daemon model picker instead of prompting), `Composer.test.tsx` (`/model` built-in), `WorkspacePanes.test.tsx` (picker stage on the same sheet), `src/server/agent_picker.rs::every_picker_stage_is_a_palette_session_with_its_mode` |
...
| C62 | Agent-less tab: open a tab with no agent, inspect the lane, then attach an agent from the lane's picker | The lane keeps its place (the view pane's chrome strip, the rails at full height — plan 125; see module 13 S47 for the open D7 width deviation) with the composer made typable and the placeholder `Type a prompt — attach an agent to send it`; model/effort/meter are hidden; the agent type trigger reads `Attach an agent`. Choosing a profile switches to the attached lane (controls + meter appear, `Ask, or type / or @`) without leaving the view; a tab that arrives without an agent adopts the default type on its own (C66) | `AgentLane.test.tsx` (agent-less vs attached; keeps typing open with no provider and states the reason in the foot), `Composer.test.tsx` (agent-less placeholder), `WorkspacePanes.test.tsx` (agent-less state from the approved artifact) |
...
| C64 | Negative: on a tab with no provider, submit a prompt; submit an unknown slash command; hide the lane while the palette is open; open the palette on a tab whose lane is hidden; open a picker stage and try to type the credential into the composer | The prompt is refused with the configuration error and **no** transcript turn or provider call happens; the unknown slash command reports fail-closed with no residue; hiding the lane closes the palette and its veil (no orphan scrim — plan-124 launch-test defect D6); the palette trigger on a lane-hidden tab makes the lane visible first, then opens the sheet; and in a picker stage the composer field is disabled/cleared, so a credential can never be typed into the persisted draft (its `/` sigil is not drawn either). Nothing in the lane path runs package JavaScript | `AgentLane.test.tsx`, `Composer.test.tsx` (unknown slash; closes the palette when the slash goes away; closes the @ menu while a stage is up), `WorkspacePanes.test.tsx` (D6 veil coupling; picker stage shields its credential there), `package_command_lane_*` denial suites, `src/server/agent_picker.rs::secret_is_not_in_snapshot_query_or_labels` |

## Plan 125 steps (picker flows through the palette, 2026-09-18)

Deep references: `DESIGN.md` §12 (transient surfaces),
`plans/125-Composer-Palette-Stage-Flows-and-Centered-Sheet-Retirement.md`,
`design-artifacts/approved/composer-palette-stages/`,
`docs/reference/ui-components.md`, `src/server/agent_picker.rs`.

| # | Action | Expected | Automated leg |
|---|--------|----------|---------------|
| C65 | From the lane, run `/resume`, `Choose Provider`, `Configure Provider`, `Choose Agent` and `Choose Model` in turn, and step a provider flow down to its credential stage | Every one of them is a **stage of the same bottom sheet**: prompt/verb/rows swap in place (`Session`, `Provider`, `Agent type`, `Model`, `Credential`), the sheet keeps its rect and its 6px gap, and the same veil stays up. A stage never opens a centered dialog, never draws a second input (only the credential stage draws its own shielded field), and `Esc`/`Alt+Left` walk back one stage at a time (closing the session at the flow entry, even from a filtered list). | `Composer.test.tsx` (walks a stage back with Esc and Alt+Left, and cancels only the catalogue; types the credential in the sheet), `CommandPalette.test.tsx` (draws a picker stage with its prompt, its filter and its own keys), `WorkspacePanes.test.tsx` (draws a picker stage on the same sheet and shields its credential there), `src/server/agent_picker.rs::stage_back_derives_the_previous_stage_and_drops_the_secret`, `flow_entry_is_the_picker_list_alone`, `src/server/menu_sessions.rs::picker_backspace_walks_the_flow_and_closes_at_its_entry`, fixture tool `design-artifacts/tools/capture-lane-palette.mjs` |
| C66 | Create a fresh tab on a workspace with no agent and watch it without touching the picker | The tab adopts the default agent type (preferring `coding-agent`) as soon as the listing lands: the picker shows it, the controls/meter appear, and the tab's label becomes the agent's name. An already-attached tab is untouched, and no second pick or session is created. | `WorkspacePanes.test.tsx` (adopts the default agent for a tab with none, once the listing lands; leaves an already-attached tab alone, with no extra pick), `AgentLane.test.tsx` (hands the picked type to the tab), `Composer.test.tsx` (`defaultAgentType` preference) |
| C67 | On a tab with no agent, type a prompt and press `Enter`; then type `/model` and `@` | The field is typable with no agent: submitting reports the error and no turn/provider call happens; `/` still opens the palette (the catalogue is available without an agent) and `@` still opens the mentions menu, so the lane is never a dead end. The composer never becomes read-only while the lane is visible. | `AgentLane.test.tsx` (offers the agent picker and keeps the field typable with no agent; sends a prompt on Enter), `Composer.test.tsx` (agent-less placeholder; submits what was typed when the palette has no rows to run), `WorkspacePanes.test.tsx` |
| C68 | Negative: with the agent view and the lane both up, switch the tab's view, toggle the lane, and quit/relaunch the tab; then open a credential stage and inspect the draft | The picker stages are shell state, not agent-view state: the agent view keeps rendering the same session while a stage is up, a tab switch cancels the stage (one session per tab), and nothing about a credential stage survives a relaunch — the composer draft comes back empty and no masked or unmasked secret is written to `layout.json`. | `WorkspacePanes.test.tsx` (session cancel on tab switch; one sheet for both views), `frontend/src/shell/tab-store.test.ts` + `src/shell/layout_persist.rs` (layout fields, no secret), `src/server/agent_picker.rs::secret_is_not_in_snapshot_query_or_labels`, `src/server/menu_sessions.rs::cancel_drops_session_and_unknown_ids_stay_unknown` |
...
| Step | Result | Evidence |
|---|---|---|
| C59 (model picker via the palette) | PASS live (trigger states) + PASS automated (palette route) | Live (canonical config): the lane's model trigger is the disabled `Configure a provider` row with the foot at `no provider configured · Settings · Providers`, exactly as designed for an unconfigured provider. The `/model` → palette `Model` stage route and the mode/verb are pinned by `AgentLane.test.tsx` (`/model` opens the daemon model picker instead of prompting), `Composer.test.tsx`, and `every_picker_stage_is_a_palette_session_with_its_mode`; switching models with a configured provider stays a configured-environment step (module 17's standing note). |
| C62/C64 (agent-less lane, negatives) | PASS live + PASS automated | Live: the agent-less lane kept its place with a typable `entry Message`, the picker read `Attach an agent` before adoption, and hiding the lane with the sheet open removed sheet **and** veil (D6). Automated: `AgentLane.test.tsx`, `Composer.test.tsx`, `WorkspacePanes.test.tsx`. |
| C65 (picker stages through the palette) | PASS automated + fixture / UNRESOLVED live | All seven stage scenes render in one sheet with the stage prompt/verb (`capture-lane-palette.mjs`, 291 checks); the stage back/entry rules and the shielded credential stage are pinned by `stage_back_derives_the_previous_stage_and_drops_the_secret`, `flow_entry_is_the_picker_list_alone`, `picker_backspace_walks_the_flow_and_closes_at_its_entry`, `Composer.test.tsx`, `CommandPalette.test.tsx`, `WorkspacePanes.test.tsx`. Live row activation is unreachable on this host (no input synthesis, rows expose no AT-SPI action), so the live leg is recorded as UNRESOLVED rather than claimed. |
| C66 (default agent adoption) | PASS live + PASS automated | Live: a fresh tab showed `Coding Agent` in the picker and the tab label `workspace agent` before any user pick (`screenshots/01-rest.ax.txt`); `WorkspacePanes.test.tsx` pins the once-only adoption. |
| C67 (typable with no agent) | PASS live (typable) + PASS automated (submit/`@`) | Live: `entry Message` present and enabled on the agent-less tab (never read-only). Submit-with-no-agent error, `/` and `@` availability: `AgentLane.test.tsx`, `Composer.test.tsx`. |
| C68 (no stage/secret survives) | PASS automated | `WorkspacePanes.test.tsx` (stage cancelled on tab switch, one sheet for both views), `secret_is_not_in_snapshot_query_or_labels`, `cancel_drops_session_and_unknown_ids_stay_unknown`, layout persist tests (no secret field). Live relaunch leg not driven (host ceiling). |
