...
## Plan 136 third-party latency-lane step

Deep references: `docs/development/performance.md` (plan 136 lane-occupancy
section), `test-plan/artifacts/136-capability-grants/README.md`. Setup: the plan
136 fixture (`@fixture/lane`) granted through `clay package authorize` and
loaded by the isolated harness (`run-live.sh start granted-lane`); its
`parser.js` holds the general lane for `BUSY_MS` while its module-backed
completion provider answers on the latency lane.

| # | Action | Expected |
|---|--------|----------|
| Q44 | With the granted fixture loaded and its parse handler holding the general lane busy (`BUSY_MS` 500–1000 ms), request completion from its module-backed provider, then stop the server with `SIGTERM` and read the summary (`CLAY_PERF_PROFILE=1`, `CLAY_PERF_REPORT_DIR`) | The third-party provider is served from its own latency lane while the general lane is busy: measured completion latency stays in the tens-of-microseconds-to-low-milliseconds envelope (this run `busy_ms=500 idle_median_us=1619 busy_completion_us=2937 workers_started=4`, inside the plan 127 record of 1567/2594 µs), typing stays local (`server.edit_ack` p50 165.0 µs / p95 191.3 µs over 6 edits against the plan 136 baseline p50 231.8 / p95 282.1 µs), and the summary carries the per-(domain,lane) occupancy counters for the first time (`js_runtime.lane.third_party.latency.dispatched` 3 = the provider registration, `js_runtime.lane.third_party.general.dispatched` 5 = the parse-handler registration, `trusted.general.dispatched` 2, with `pending`/`peak_pending`/`superseded`/`evicted` reported for all four lanes). Negatives: no keystroke waits for the parse handler, no supersede/evict churn outside the burst tests, no package work on a trusted lane, and the ungranted fixture still fails closed instead of contributing a provider. Ceiling: the live popup cannot be driven — package-owned modes do not activate for open documents and the package mode rejects the built-in `completion.trigger` command — so the latency numbers come from the automated lane harness (`cargo test --lib lanes_and_queues -- --nocapture`, 17 tests) and the live leg contributes the edit-ack envelope plus the occupancy counters. |
