...
const root = new URL("../../tests/fixtures/lsp/typescript/", import.meta.url).pathname.replace(/\/$/, "");
const identity = {
...
test("real typescript-language-server maps TypeScript semantic, diagnostics, completion, hover, definition, and shutdown", {
  skip: !enabled,
  timeout: 45000,
}, async () => {
  ...
  try {
    ...
    assert.ok((diagnostics.at(-1)?.spans.length ?? 0) > 0, "TypeScript diagnostic expected");
