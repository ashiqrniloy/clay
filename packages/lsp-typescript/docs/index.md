...
Opt-in `typescript-language-server` intelligence for documents already using the `typescript` mode from `@clay/typescript`.
...
Bridge starts lazily for first eligible `.ts`/`.tsx`/`.mts`/`.cts` document. It negotiates rather than assumes server capabilities, uses the selected UTF-8/UTF-16/UTF-32 position encoding (UTF-16 default when omitted), and synchronizes accepted open/change/reset/close versions. Current `typescript-language-server` support maps:

- full semantic tokens to priority-100 semantic decorations;
- pushed diagnostics to source-keyed `lsp-typescript` diagnostics;
...
Completion stays priority 100 and non-exclusive so `@clay/typescript` keywords/snippets remain merged. Use `serverDisableCompletion({ provider: "lsp-typescript.completion" })` or a base provider id/prefix when you want an explicit override.
...
- No LSP output: verify directory root ID is current and document uses `typescript` mode. Base package remains active while bridge is unavailable.
