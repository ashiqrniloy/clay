...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        print(json.dumps({"jsonrpc":"2.0","id":ident,"result":{"maxInputTokens":params.get("maxInputTokens",None),"maxOutputTokens":params.get("maxOutputTokens",None),"maxTurns":params.get("maxTurns",None),"maxToolRounds":params.get("maxToolRounds",None),"maxToolCalls":params.get("maxToolCalls",None),"maxWallTimeMs":params.get("maxWallTimeMs",None),"compactAfterTokens":params.get("compactAfterTokens",800000),"compaction":params.get("compaction","llm")}}), flush=True)
...
#[cfg(unix)]
#[tokio::test]
async fn run_set_options_forwards_to_daemon() {
    let host = host_for(mock_daemon());
    let result = host
        .rpc(
            "run.setOptions",
            json!({ "maxInputTokens": 500_000, "compaction": "om" }),
        )
        .await
        .expect("run options forward");
    assert_eq!(
        result.get("maxInputTokens").and_then(Value::as_u64),
        Some(500_000)
    );
